...
`CHECKLIST_V395.md`. Response letter: `REFEREE_RESPONSE_V395.md`. Every
...
`stageonenull_v395.py` imposes both conditions. The per-window
...
`occurrence_v395.py`, added for R2-2, wrote the file
`stageonenull_v395.py` owns. The build reported 0 errors and 0 undefined
...
v3.95 generators look their inputs up by name in the round files
...
| file | purpose |
|---|---|
| `stageonenull_v395.py` | the correctly-conditioned stage-1 null (R1-1, R1-4) → round 47 |
| `falsealarm_v395.py` | one table of all seven expectations, with `predicts` (R1-1) → round 46 |
| `occurrence_v395.py` | CWTFM, Transmitter Rate, hosting fraction (R2-2) → round 48 |
| `rfi_v395.py` | three RFI vetting measurements (R2-3) → round 49 |
...
`occurrence_v395.py` cross-asserts its inputs against `NSysClassA`,
...
Unchanged from v3.86, plus: `stageonenull_v395.py` **before**
`falsealarm_v395.py` (which reads its macros), and both before
`stagetable_v386.py`. `occurrence_v395.py` and `rfi_v395.py` read only the
...
literal, v3.62 printed frozen-input arithmetic, and v3.95 printed a ledger
