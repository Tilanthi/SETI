# Version history — "A Search for Spectral Technosignatures in the Public
# ALMA Archive: 82 Stellar Systems within 40 Parsecs"

(The title changed from "within 20 pc" at v3.61, when a referee
noticed the manuscript stem and the title disagreed. This heading was
not updated until v3.95.)
...
**Not recorded here.** Each of those cycles wrote its own
`BUILD_NOTES_v3NN.md` and `REFEREE_RESPONSE_V3NN.md` in its version
folder, and this file was not kept up. Noted at v3.95 rather than
reconstructed, since the per-version notes are the primary record and are
complete.

## v3.95 (2026-09-21) — two referee reports, 26 recommendations, none declined

Full notes: `BUILD_NOTES_v395.md`. Letter: `REFEREE_RESPONSE_V395.md`.
Ledger: `CHECKLIST_V395.md`.

**The change that alters a claim.** Every false-alarm expectation the
paper quoted predicted *rank-first windows*, while the observed quantity
is *unattributed stage-1 events*, which require the trigger as well as the
rank. Imposing both conditions (`stageonenull_v395.py`, block-resampled
20,000 times with the measured x1.4 tail factor) gives **4 observed
against 1.07 expected, P(>=4) = 0.025**, replacing "a modest population
excess" in the abstract, S5.3 and the conclusions.

The same calculation dissolves the reference-class question: only 460 of
1614 windows can reach the trigger from noise at all, and **391 of those
are Class A**, which is why every stage-1 event in the survey is Class A.
The superseded `tab:chance`, which existed to display that ambiguity, is
deleted.

**Structural.**
- P_90^sel is now the single headline sensitivity everywhere. Table 1's
  power block is an ordered hierarchy; the glossary's duplicate P_trig,
  P_eff, P_90 and stage-1 rows are deleted; the Mason et al. (2024)
  comparison is recomputed on it (x459, with x595 on P_90 beside it).
- The candidate definition is reordered to threshold -> spatial
  prioritisation -> **visibility localisation** -> line identification ->
  recurrence, with the first two named as candidate generation.
- Class A and Class B are two experiments in the conclusions, with no
  combined figure.
- "Work-list complete but epoch-incomplete" is the only completeness
  formulation for the sample.
- The methodological defence leaves the Introduction for S4.2 and the
  appendices, where it already existed in full.

**New measurements.**
- Standard occurrence metrics (`occurrence_v395.py`): Transmitter Rate
  0.17, **CWTFM 1.3e4** at the median per-system P_90^sel against
  Enriquez+17's 0.85, and f < 5.0 per cent hosting fraction at 95 per cent
  Poisson. The paper states plainly that this is four orders of magnitude
  worse than cm-wave SETI on the standard metric, and why.
- RFI vetting (`rfi_v395.py`): 0 same-channel coincidences with unrelated
  stars; crossing frequencies show no clustering at 1 MHz (max
  multiplicity 2 against 1.9 expected, p = 0.90); fractional positions
  within windows uniform to D = 0.023, p = 0.97 over 451 crossings, so no
  fixed-intermediate-frequency artefact.
- sigma is now reserved for genuine noise scales; the trigger is
  T_trig = 5 and the power that reaches it EIRP_trig.

**Defects found and fixed while doing the above:** a false-alarm table
whose header did not match its body; a printed subtraction that did not
close (27.10 - 26.83 = 0.27, printed 0.28); the deepest repeat of the four
unattributed events crossing the trigger at T* = 5.42, which the "none
recurs" claim had not admitted; and appendix G.10 mistitled, apparently
duplicative and never referenced despite carrying 104 unique macros.

**Two process defects, both now gated:** `occurrence_v395.py` silently
overwrote `survey_numbers_round47.tex`, which `stageonenull_v395.py` owns,
and the build reported no error because the destroyed macros survived in
the `.aux`; and `retire_macros.py`, not knowing that generators read
macros back out of the round files, retired 13 and turned four table rows
into `--`. New gates `roundcollide.py` and the reader-protection pass in
`retire_macros.py`; new `xrefcheck.py` for cross-reference resolution,
which found two labels anchored on tables rather than sections.

Gates: **45 pages**, 0 errors / 0 undefined / 0 multiply-defined /
0 overfull / 0 Type 3, 32 underfull, 1107 macros 0 unused, abstract
**1917/1920**, arXiv **86 items**, clean regeneration **88/88
byte-identical** including 14/14 figures, number audit 49/49, catalogue
self-sufficiency 36/36.


## v3.95 (2026-09-22) — the prospective epoch extension

Full notes: `BUILD_NOTES_v395.md`.

**123 public in-scope execution blocks that the frozen survey never
searched were found and searched.** They existed because
`make_worklist.py` had been built against the v3.43-era archive map (102
member OUS, 448 progenitors); the v3.81 map (152 OUS, 656 progenitors)
exposed them. They were searched with the frozen pipeline AFTER the
statistic and the line mask were fixed.

**They are deliberately NOT merged into the science sample.** Every
statistic in the paper rests on a denominator and a candidate list fixed
before the data were examined; merging would destroy that. The frozen
sample is unchanged and the new data is reported as a prospective
extension with its own null, its own table and its own released file.

After applying the survey's own eps Eri Band 6 withholding rule (all 32
such windows withheld, as in the frozen sample): **115 blocks, 463
windows, 16 stars, 16 systems, 57.6 GHz**.

Three results:
1. **The exchangeability failure reproduces on unseen data** — median
   add-one rank **0.409** over 463 windows against 0.5 expected,
   D = 0.116. A third independent measurement after the hold-out (0.429)
   and the radius analysis (0.437).
2. **The positive control recovers** — 3 stage-1 events, all three the
   beta Pictoris disc CO(1-0) at T* = 23.17 / 19.82 / 19.39 against ring
   maxima below 6.21, in three DIFFERENT previously unsearched blocks.
3. **No new unattributed event** — 13 crossings, 4 rank-first against 1.3
   expected, 0 unattributed against 0.21 expected. The paper's
   unattributed count remains 4.

The epoch gain is small and is reported as such: systems with more than
one searched epoch 55 -> 56 of 82, single-epoch systems 27 -> 26. That
matches the prediction S6.4 item (2) made before the campaign ran.

Gates: **47 pages**, 0 errors / 0 undefined / 0 multiply-defined /
0 overfull / 0 Type 3, 29 underfull, 1149 macros 0 unused, abstract
**1912/1920 on both counters**, clean regeneration **90/90
byte-identical**, number audit 49/49, catalogue self-sufficiency 36/36,
arXiv set 87 items.

Traps: the v387->v395 rename orphaned `tab_falsealarm_v387.tex` and only
the clean-regeneration test saw it; the two abstract counters disagreed
again (source 1917 vs PDF 1926) and the PDF is what arXiv reads; and a
first draft of `tab:extension` used one macro for two different frozen
quantities, fixed by computing every frozen number from the catalogue and
asserting it against the published macros.
