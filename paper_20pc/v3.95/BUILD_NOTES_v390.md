# BUILD NOTES — v3.95: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v395.py`, now asserts that
...
to do if it failed. New `radval_v395.py` does exactly that on
