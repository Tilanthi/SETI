...
python3 stageonenull_v387.py        > /dev/null   # -> survey_numbers_round47.tex (R1-4: the null for UNATTRIBUTED STAGE-1 events, trigger condition included)
python3 falsealarm_v387.py          > /dev/null   # -> survey_numbers_round46.tex, tab_falsealarm_v387.tex (R1-1); must follow stageonenull
python3 occurrence_v387.py          > /dev/null   # -> survey_numbers_round48.tex (R2-2: CWTFM / Transmitter Rate / hosting fraction); cross-asserts NSysClassA, UnionClassA, SysUnionAMed
python3 rfi_v387.py                 > /dev/null   # -> survey_numbers_round49.tex (R2-3: RFI vetting of the unattributed events)
