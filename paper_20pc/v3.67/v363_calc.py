...
M('PbFloorResp', '0.5')            # response floor adopted in v3.67
...
# =====================================================================
# (10) uncertainty on the measured tail correction (referee 2, M4)
# =====================================================================
# The referee supposed the factor rested on "of order 1-2 exceedances".  It
# does not: the pseudo-star rate is measured on NPseudo trials, so the Clopper-
# Pearson interval is narrow.  Computed here from the frozen numbers.
_CI = json.load(open('tailratio_ci_v367.json'))
M('TailRatioMeas', '%.2f' % _CI['ratio'])
M('TailRatioLo', '%.2f' % _CI['ratio_ci'][0])
M('TailRatioHi', '%.2f' % _CI['ratio_ci'][1])
M('TailNTrials', '{:,}'.format(_CI['n_pseudo']).replace(',', '\\,'))
M('TailNExceed', '%d' % _CI['k'])
