...
-------
  ...
  archive_meta_v381.json       ALMA obscore + datalink harvest: progenitor
  ...
  cp72_recurrence.json    the CP-72 2713 second-epoch generator output.
...
AM = json.load(open('archive_meta_v381.json'))
ROWS = list(csv.DictReader(open('per_target_results_v3.99.csv')))

# The recurring beta Pic Band 6 CO(2-1) tuning. The repaired extraction
# re-measures each peak, so this is matched within a physical tolerance
# rather than by an exact literal.
BPREC_SIX = 230.5284
BPREC_TOL = 1e-3
CP = json.load(open('cp72_recurrence.json'))
...
# v3.80: this block WAS the one the paper said was queued and disk-gated.
# The completed sweep searched it, so the unit is now exhaustive and the
# manuscript's caveat has to go. Emit the state rather than assert the old
# one; \BpFlagSixNUnsearched is what the text should read.
M('BpFlagSixNUnsearched', '%d' % len(_unsearched))
M('BpFlagSixNProg', '%d' % _f['n_progenitor_ebs'])
M('BpFlagSixNSearched', '%d' % len(_searched))
if _unsearched:
...
for h in _b6:
    ...
    if v is None:
        _cat = [r for r in ROWS if r['eb'] == h['eb']
                and abs((F(r['f_cross_GHz']) or 0) - BPREC_SIX) < BPREC_TOL]
...
_row6 = [r for r in ROWS if 'bet Pic' in r['star_name']
         and abs((F(r['f_cross_GHz']) or 0) - BPREC_SIX) < BPREC_TOL]
if len(_row6) > 1:                      # keep the closest if several match
    _row6 = [min(_row6, key=lambda r: abs(F(r['f_cross_GHz']) - BPREC_SIX))]
assert len(_row6) == 1, (
    'the recurring beta Pic Band 6 window was not found within %.0f kHz of '
    '%.6f GHz (%d matches). The repaired extraction re-measures this peak, '
    'so it must be matched on proximity, not on an exact literal.'
    % (BPREC_TOL * 1e6, BPREC_SIX, len(_row6)))
_row6 = _row6[0]
...
# the RECURRING Band 6 window, from its own two blocks: 244140.625 Hz channels,
# 32 and 28 channels above threshold.  v3.47 printed \BpWidthSixKms in this
# sentence, which is the STAGE-1 window's 114 crossings at its 15.26 kHz
# channel width, i.e. a different dataset (R1-N1, R2-E1, R3-E17).
_w6r = [h['n_hits_above_threshold'] * h['chanwidth_Hz'] * 1e-9
        / h['star_peak_freq_GHz'] * C for h in _b6]
M('BpWidthSixRecLo', '%.1f' % min(_w6r))                 # 8.9
M('BpWidthSixRecHi', '%.1f' % max(_w6r))                 # 10.2
...
# R1-E3 (v3.49): select B alone.  'G 272-61' in star_name also matched the A
# component, pooling both into the window count and taking A's row as the
# deepest, while the prose quotes B's distance and EIRP.
_uv = [r for r in ROWS if r['star_name'] == 'G 272-61B' and r['band'] == '3']
...
# The grid over-count, DERIVED rather than asserted (referee C2, v3.54).  Up to
# v3.50 this was the hand-set constant 4.0 with the comment "Hanning in
# frequency x drift oversampling"; the arithmetic behind it was never shown and
# never checked.  Frequency: the Hanning kernel correlates neighbouring
# channels, so the variance of a sum over n channels exceeds n by
# 1 + 2 rho1 + 2 rho2 = 8/3, which is how many grid channels carry one
# channel's worth of independent information.  Drift: the released extraction
# code steps the drift grid at DRIFT_STEP_DIV = 2 points per channel of
# traverse over the track, so adjacent trials count 2 for 1.  The axes are
# independent, so the product is the over-count: 8/3 x 2 = 5.33, not 4.
_RHO1 = sum(_h[i] * _h[i + 1] for i in range(2)) / _den
_RHO2 = (_h[0] * _h[2]) / _den
DRIFT_STEP_DIV = 2.0
OVERCOUNT = (1.0 + 2.0 * _RHO1 + 2.0 * _RHO2) * DRIFT_STEP_DIV
