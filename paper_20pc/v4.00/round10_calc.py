...
# Guard: nothing here may move except by an audited change.  The v3.39-v3.41
# values were 6.2 / 59 / 3.6 / 8.6 / 10.8, computed with HD 139084B counted as
# two independent systems.  Referee A's first point was that this is one system
# double-counted; v3.46 adds it to PAIRS in survey_stats_round10.py, so
# N_sys 82 -> 81 and every per-system denominator moves with it.  The guard is
# re-anchored on the corrected values here, in the same commit as the fix, and
# still aborts on any drift away from them.
# v3.60 re-anchors two of the five, for the second audited reason in the
# paper's history: the twelve Band 9 and Band 10 windows dropped by the
# catalogue exporter's band regexp are now in the released catalogue
# (443 windows, not 431).  They are wide (1.7 GHz each) and shallow
# (4.1e15-3.0e16 W), and the occurrence estimator is a BANDWIDTH-WEIGHTED MEAN
# completeness per system, not a sum, so adding coverage that is insensitive
# at 1e16 W lowers AU Mic's and HD 61005's weighted recovery and therefore
# WEAKENS the limit slightly: 6.3 -> 6.4 per cent, and the duty-cycle
# variant 11.0 -> 11.2.  The other three are unmoved to the digit.
# v3.80 re-anchors ALL FIVE, for the third audited reason in the paper's
# history and the first that improves the science rather than correcting it.
# The catalogue grows from 443 to 1895 windows and 104 to 482 execution
# blocks as the archival sweep completes, and 65 systems now carry
# drift-resolving (fine-channel) coverage where 58 did before.  Because the
# estimator is a bandwidth-weighted mean completeness per system, deeper and
# broader coverage of the SAME systems raises the weighted recovery and so
# TIGHTENS the limit: 6.4 -> 5.9 per cent, and the duty-cycle variant
# 11.2 -> 7.6.  Direction matters here.  The v3.60 re-anchor went the other
# way (wide, shallow Band 9/10 windows weakened it), which is the check that
# this estimator responds to what was actually added, not to the count.
# v3.81 re-anchors two of the five on the pre-registered hold-out split.
# Reserving 77 of 484 blocks removes coverage from the survey, so the
# bandwidth-weighted mean completeness per system falls slightly and the
# limit LOOSENS: 5.86 -> 5.84 per cent and the duty-cycle variant
# 7.61 -> 7.75. That direction is the check. Holding data back cannot make
# a survey more complete, and if this guard had moved the other way the
# split would be wrong. The system count and the two coverage-free
# variants are unmoved, as they must be: no system is lost to the hold-out.
for got, want, what in ((100 * O16['measured'], 5.84, 'OccMeasured'),
                        (O16['n_sys_fine'], 65, 'NOccSystems'),
                        (100 * O16['unit'], 3.42, 'OccUnit'),
                        (100 * O16['uniform'], 8.15, 'OccUniform'),
                        (100 * O16['measured_D05'], 7.75, 'DutyHalf')):
...
# ============================================================================
import math as _m
_d = json.load(open('corrected_export_v399.json'))   # v3.80: was v3.31
_rows = _d['rows']
def _band(r):
...
# v3.80: keyed on (star, band) and now holding 7 star-band pairs over 13
# windows, since beta Pic recurs in five Band 3 and three Band 6 windows.
# Keep the deepest window of each pair, which is what the flux macros below
# have always meant, and assert against the count the catalogue reports
# rather than a literal.
FLAG = {}
for r in GOOD:
    if not (r['star_snr'] >= 5.0 and r['star_snr'] > r['cmax']):
        continue
    k = (r['star_name'], r['band_x'])
    if k not in FLAG or r['star_snr'] > FLAG[k]['star_snr']:
        FLAG[k] = r
_NW = sum(1 for r in GOOD if r['star_snr'] >= 5.0 and r['star_snr'] > r['cmax'])
assert _NW == _K['n_flagged'], (_NW, _K['n_flagged'])
M2('FlagPairs', '%d' % len(FLAG))
...
# Single-sourced with the barycentric concordance of v346_calc.py; bpic_vsys.py
# records why the Gaia DR3 catalogue value is not the one adopted for this star.
from bpic_vsys import VSYS_BPIC_KMS as VSYS_BPIC, VSYS_BPIC_ERR_KMS as _VSYS_ERR

M2('VsysBpic', '%+.1f' % VSYS_BPIC)
M2('VsysBpicErr', '%.1f' % _VSYS_ERR)
# The barycentric terms are tabulated; emit them so the table cannot carry a
# different number from the chain that consumes them. The topocentric
# velocity column was hand-typed as -27.46 for the Band 3 row while the
# frequency offset in the same cell gives -28.29, which is also the value the
# tabulated stellar-frame offset needs -- the row disagreed with itself.
for _t, _b in BARY.items():
    M2('Bary' + _t, '%+.2f' % _b)
...
# signed: on the adopted systemic velocity these three straddle zero, so the
# sign has to be printed explicitly or the table reads as three positive offsets
...
# app:linemask quotes the largest attributed circumstellar excursion as a
# fraction of the mask half-width; both were hand-typed before v3.47 and both
# move when the adopted systemic velocity does.
_MASKHALF = 50.0
_bpmax = max(abs(_stel3), abs(_stel6), abs(_stelc))
M2('BpicMaxAbsStel', '%.2f' % _bpmax)
M2('BpicMaskPct', '%.1f' % (100.0*_bpmax/_MASKHALF))
