...
STEPS = [
    ...
    # The ledger must be NESTED: every step is a subset of the one above it.
    # It previously placed "localised in the visibilities" (a vetting outcome
    # over all flagged windows, most of them molecular) above "without an
    # astrophysical attribution", and the second set is not contained in the
    # first -- so the drawing implied a funnel it did not have. The
    # attribution split comes first, and the visibility test is then applied
    # to what survives it.
    ("Without an astrophysical attribution", n("NStageOneUnattrib"),
     "events"),
    ("Of those, localised at the star in the visibilities",
     n("NVisPointSource"), "events"),
...
# R1-11 (v3.99): three fundamentally different operations, named as the
...
# "ten steps" against eleven drawn until v3.99.
