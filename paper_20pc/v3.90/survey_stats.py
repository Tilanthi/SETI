...
SRC='frozen_export_v3.81_survey.json'   # v3.46: was an absolute path outside the
...
# v3.85: alias-merge before the pair collapse, or a star entered under two
# name strings counts twice. Shared with v342_calc.py and the table and
# figure generators via star_alias.py.
from star_alias import sysname as _sysname, canon as _canon
sysn = _sysname
...
stars=sorted({_canon(r['star_name']) for r in good}); systems=sorted({sysn(s) for s in stars})
S['n_stars']=len(stars); S['n_systems']=len(systems)
S['n_starbands']=len({(_canon(r['star_name']),r['band_x']) for r in good})   # v3.85: canonical
...
# ---------------------------------------------------- catalogue constants
# v3.80.  Until now each generator carried the catalogue's size as a literal
# ("assert len(good) == 431"), and those literals were the paper's most
# persistent source of error: a later round enlarges the catalogue, one
# generator is repointed and another is not, and the manuscript prints two
# mutually inconsistent totals with an assertion passing in between.  The
# assertions were doing something useful -- they catch a generator reading a
# different catalogue from its neighbours -- so keep the check and remove the
# literal.  Every generator now asserts against these, written once, here,
# from the same filtered sample the paper quotes.
K = dict(n_windows=len(good), n_fine=S['n_fine'], n_coarse=S['n_coarse'],
         n_stars=S['n_stars'], n_systems=S['n_systems'], n_eb=S['n_eb'],
         n_starbands=S['n_starbands'], n_withheld=len(withheld),
         n_defect=len(defect), n_dup=n_dup, n_flagged=S['n_flagged'],
         n_flagged_old=S['n_flagged_old'], n_src=S['n_src'],
         n_ctrl=S['n_ctrl'], source=SRC, snapshot=d.get('snapshot'))
json.dump(K, open('catalogue_constants.json', 'w'), indent=1)
