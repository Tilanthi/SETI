...
shipped here:
  ...
  Fig. 5  <- localnull_code/ctrlmax.json   (the window's 512 control statistics)
            per_target_results_v3.61.csv   (T_star and the ring maximum)
  Fig. 7  <- per_target_results_v3.61.csv  (window centre, nominal EIRP, class)
...
def fig_sensitivity_2d():
    ...
    axr.set_ylabel(r"effective threshold $P_{\rm eff}=%.2f\,P_{\rm trig}$ (W)"
                   % _han, fontsize=10)
