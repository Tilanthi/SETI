...
Inputs (all frozen; all in this directory unless noted):
  frozen_export_v3.60.json        the per-window search export (512 controls)
  per_target_results_v3.61.csv    the released catalogue (431 windows)
  ...
  technosignatures_40pc_v3.61.tex parsed, read-only, for the transfer bracket
...
F = float
ROWS = list(csv.DictReader(open(os.path.join(HERE, 'per_target_results_v3.61.csv'))))
...
def load_windows():
    ...
    d = json.load(open(os.path.join(HERE, 'frozen_export_v3.60.json')))
    rows = d['rows']
...
CATALOGUE_FILE = 'per_target_results_v3.61.csv'
