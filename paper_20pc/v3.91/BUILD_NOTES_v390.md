# BUILD NOTES — v3.91: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v391.py`, now asserts that
...
to do if it failed. New `radval_v391.py` does exactly that on
