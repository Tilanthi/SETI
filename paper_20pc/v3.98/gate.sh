...
echo "--- generator/round-file ownership (v3.98: added after a silent overwrite) ---"
python3 roundcollide.py

echo "--- cross-reference resolution (R2-t2) ---"
python3 xrefcheck.py

echo "--- primary-statistic consistency (referee 2, v3.98) ---"
python3 consistency_v398.py

echo "--- macro leakage (referee 2, v3.98) ---"
echo "--- prose literals vs macros (referee 2, M6) ---"
python3 prosenum_v398.py
