...
Clean regeneration **81/81 byte-identical**. New gate `prosenum_v398.py`: 0.
...
**Three measurements confirm the mechanism** (`acageom_v398.py`, round 61):
...
| R2-M6 | Fixed every stale count: three "one observed"/"the single unattributed" left from when the survey had one event; **Table 7's recurrence criterion**, which tested the trigger alone and so marked HD 14055 as recurring on a repeat that does not outrank its controls. Criterion now matches the stage-1 definition. New gate `prosenum_v398.py` checks prose literals against macros |
...
| R1-4 | **Sensitivity is now a distribution** (`sensdist_v398.py`): of 60 systems, 3 reach 10¹⁴ W, 22 reach 10¹⁵ W, 55 reach 10¹⁶ W and all 60 reach 10¹⁷ W. In the abstract |
...
`acageom_v398.py` (round 61) — asserts the implemented inner radius lies inside
the ACA synthesised beam, and that the excess is confined to the ACA stratum.
`sensdist_v398.py` (round 62) — asserts the benchmark counts are consistent
with the quoted median. `prosenum_v398.py` — gate, prose literals vs macros.
`epochsplit_v398.py` — now asserts the temporal partition closes.
