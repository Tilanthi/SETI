...
Writes `tailboot_v372.json`.  No network, no products beyond this folder.
"""
...
# v3.72: the vector store is now the full calibration-sample harvest
# (`ctrl_calib_v372.json`, keyed EB_spwN), 1326 windows against the 603 of
# `heldout_ctrl_v361.json`.  Every window ever searched has its 512-probe
# vector in its `*_search.npz`; the pipeline also writes `ctrl_all` into the
# result JSON since 2026-09-14.
HO = [{'eb': k.split('_spw')[0], 'c': v}
      for k, v in sorted(json.load(open('ctrl_calib_v372.json')).items())]
win = []
for r in HO:
...
json.dump(out, open('tailboot_v372.json', 'w'), indent=1)
