# BUILD NOTES — v3.98: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v398.py`, now asserts that
...
to do if it failed. New `radval_v398.py` does exactly that on
