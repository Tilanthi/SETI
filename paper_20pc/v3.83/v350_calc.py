...
CATALOGUE = 'per_target_results_v3.83.csv'
RANKFLOOR = float(texval('survey_numbers.tex', 'RankFloor'))
# The Bonferroni scale is alpha over the number of windows SEARCHED, and
# the catalogue decides that. Reading `BonferroniThresh` out of the frozen
# round-5 macro file instead meant the ratio was computed against a window
# count three catalogues out of date: the paper printed a factor 16 while
# its own two printed numbers, 1.95e-3 and 3.0e-5, give 65.
NWIN = len(list(csv.DictReader(open(os.path.join(HERE, CATALOGUE)))))
ALPHA = 0.05
BONF = ALPHA / NWIN
ratio = (1.0 / RANKFLOOR) / BONF
M('BonfRankRatio', '%d' % round(ratio))
M('RankFloorVal', '%.2f\\times10^{-3}' % (1e3 / RANKFLOOR))
M('BonfNWinCheck', '%d' % NWIN)
M('BonfScaleHere', '%.3e' % BONF)
# The cross-check against the printed scale lives in v351_calc.py, which
# emits \BonfScaleCalc: reading it here would be a forward dependency and
# a clean regeneration would fail at the first generator.
...
# v3.80: the injection campaign was calibrated on one AU Mic Band 6 window
# at this channelisation; the completed sweep contains three of them, from
# three execution blocks. The calibration configuration is a (band,
# channel width, drift grid) triple, not a particular block, so take the
# longest on-source window deterministically and record how many share the
# configuration.
assert CAL, 'calibration window absent'
M('CalNWindows', '%d' % len(CAL))
CAL = sorted(CAL, key=lambda r: (-float(r['on_source_s']), r['eb']))[0]
...
# ----------------------------------------------------------------- (4)
EX = json.load(open(os.path.join(HERE, 'frozen_export_v3.81_survey.json')))['rows']
EPS = [r for r in EX if r['star_name'] == 'eps Eri' and r['band'] == 6]
# v3.80: repointed off the v3.31 snapshot with everything else. The number
# of withheld eps Eri Band 6 windows is a property of the catalogue, so it
# is asserted against the catalogue, not against four.
assert len(EPS) == _K['n_withheld'], (len(EPS), _K['n_withheld'])
M('NEpsWithheld', '%d' % len(EPS))
