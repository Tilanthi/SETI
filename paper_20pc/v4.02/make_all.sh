...
python3 vispower_v400.py            > /dev/null   # -> survey_numbers_round74.tex, tab_vispower_v400.tex (R2-M5: the POWER of the visibility test, from first principles)
...
python3 ledger_v401.py              > /dev/null   # -> survey_numbers_round76.tex, tab_ledger_v401.tex (R1-1/2/3, R2-M1: the PRINCIPAL CANDIDATE LEDGER -- the drift-following visibility fit applied to every threshold crossing, with the criterion committed before the fits were run; the 512-control rank demoted to a diagnostic column)
python3 make_fig_ledger.py          > /dev/null   # -> figures/ledger_vis.pdf (R2 Fig. 5: one Re/sigma-Im/sigma scatter over every crossing, replacing the 13 bar panels that were built from the pre-repair fit file); MUST follow ledger_v401.py, whose join it imports
