# BUILD NOTES — v3.99: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v399.py`, now asserts that
...
to do if it failed. New `radval_v399.py` does exactly that on
