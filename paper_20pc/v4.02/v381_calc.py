...
with open(TAB, 'w') as fh:
    ...
    for star, band, n, deep, dv, inside in rows:
        # A window for which the pipeline stored no peak frequency has no
        # crossing frequency to report. Printing `or 0.0` put a literal
        # 0.0000 GHz in the table, which is not a missing value but a wrong
        # one; the beta Pic Band 3 row read that way. Where the release
        # carries no peak, say so.
        _fc = F(deep['f_cross_GHz'])
        fh.write('%s & %s & %d & %s & %.2f & %.2f & %s & %s \\\\\n'
                 % (star, band, n,
                    ('%.4f' % _fc) if _fc else '--',
...
# R1-3 (v3.99): the headline quantity is now P_90^sel, so the survey
...
for r in ROWS:
    ...
    if v and v < _b90.get(r['system_id'], float('inf')):
        _b90[r['system_id']] = v
    # v4.01 (R2-M2): this read the SUPERSEDED construction
    #     eirp_p90_W x ctrl_max_snr / 5,
    # a post-baseline trigger completeness scaled by each window's control
    # maximum as a stand-in for a gate it never passed through. It survived
    # here after being replaced everywhere else, and it is why the paper
    # printed a per-system median of 1.8e15 W in one place and the measured
    # 2.9e15 W in another. The catalogue column is the single source.
    vs = F(r['eirp_p90_sel_W'])
    if vs and r['search_class'] == 'A':
        if vs < _bsel.get(r['system_id'], float('inf')):
            _bsel[r['system_id']] = vs
import statistics as _stm
_m90 = _stm.median(sorted(_b90.values()))
_msel = _stm.median(sorted(_bsel.values()))
...
m('PctGateBelowTrig', '%.1f' % (100.0 * sum(1 for c in _cmA if c < 5.0)
                                / len(_cmA)))
# v4.00 (R2-M2, R1-4): P90^sel is now MEASURED end to end rather than
# modelled. The construction it replaces was
#     eirp_p90_W x ctrl_max_snr / 5,
# i.e. the trigger-calibrated P90 from a campaign that injected into
# retained spectra AFTER the baseline step, scaled by each window's own
# control maximum as a stand-in for the rank gate it never passed through.
# The replacement injects Hanning-convolved carriers at random sub-channel
# phase and drift into calibrated VISIBILITIES and runs the unmodified
# extraction, baseline removal, de-drift stack, trigger and 512-control
# rank, scoring recovery on one criterion.
#
# THE MULTIPLIER APPLIES TO THE NOMINAL TRIGGER AND TO NOTHING ELSE. A
# measured end-to-end ratio already contains the spectral response and the
# drift smear, so applying it to eirp_eff_total_W (= P_trig C_resp C_smear)
# or to eirp_p90_W (= P_trig x 2.81, which also carries C_resp) would count
# the response correction twice. The assertion below is the guard.
_m3a = json.load(open('m3a_result_v400.json'))
_m3aS = {x['stratum']: x for x in _m3a['strata']}
SEL_RATIO = _m3aS['fine (<1 MHz)']['P90_over_trigger']
SEL_RATIO_LO, SEL_RATIO_HI = _m3aS['fine (<1 MHz)']['P90_ci68']
assert _m3aS['all']['null_recovered'] == 0 and \
    _m3aS['fine (<1 MHz)']['positive_control_frac'] >= 0.95, \
    'the selection-function campaign did not pass its own null and positive control'
_chk = [r for r in ROWS if r['search_class'] == 'A' and F(r['eirp_eff_total_W'])
        and F(r['eirp_nominal_W'])]
_cresp = sorted(F(r['eirp_eff_total_W']) / F(r['eirp_nominal_W']) for r in _chk)
_cresp_med = _cresp[len(_cresp) // 2]
assert SEL_RATIO > _cresp_med, (
    'the measured end-to-end ratio (%.2f) is below the response correction '
    'alone (%.2f); it should contain it' % (SEL_RATIO, _cresp_med))
m('SelRespMed', '%.2f' % _cresp_med)
m('SelOverResp', '%.2f' % (SEL_RATIO / _cresp_med))

# read the stored column rather than recomputing it: the catalogue is the
# single source, and a reader must be able to reproduce these numbers from
# the released file alone
_prom = sorted(F(r['eirp_p90_sel_W']) for r in ROWS
               if r['search_class'] == 'A' and F(r['eirp_p90_sel_W']))
_chkp = [F(r['eirp_p90_sel_W']) / F(r['eirp_nominal_W']) for r in ROWS
         if r['search_class'] == 'A' and F(r['eirp_p90_sel_W'])
         and F(r['eirp_nominal_W'])]
# the catalogue stores six significant figures, so compare at that level
assert (max(_chkp) - min(_chkp)) / SEL_RATIO < 1e-5 and \
    abs(_chkp[0] - SEL_RATIO) / SEL_RATIO < 1e-5, (
    'the stored selection column is not the measured ratio times the nominal '
    'trigger: %.4f vs %.4f' % (_chkp[0], SEL_RATIO))
# statistics.median, not the upper element: make_fig_classa_sens.py
# computes the same quantity with numpy and cross-asserts against it, and
# for the 60-system list the two conventions differ by 7 per cent, which is
# larger than the tolerance that check allows.
import statistics as _st
m('PromoteLoA', _sci(_prom[0]))
m('PromoteMedA', _sci(_st.median(_prom)))
m('PromoteHiA', _sci(_prom[-1]))
m('PromoteFacMed', '%.2f' % SEL_RATIO)
# what the superseded modelled construction gave, for the one sentence that
# reports the size of the change
_oldp = sorted(F(r['eirp_p90_W']) * F(r['ctrl_max_snr']) / 5.0 for r in ROWS
               if r['search_class'] == 'A' and F(r['eirp_p90_W'])
               and F(r['ctrl_max_snr']))
m('PromoteMedAOld', _sci(_oldp[len(_oldp) // 2]))
m('PromoteShift', '%.1f' % (_prom[len(_prom) // 2] / _oldp[len(_oldp) // 2]))
...
m('PromoteSysMedA', _sci(_st.median(_ps)))
...
# v4.00: P_90 is the TRIGGER completeness and is now measured end to end,
# so it is read from the catalogue column rather than reconstructed by
# scaling a response-corrected power -- which would have carried C_resp
# into a ratio that already contains it.
_fine = sorted(F(r['eirp_p90_W']) for r in ROWS
...
m('PNinetyWinMedA', _sci(_fine[len(_fine) // 2]))
m('PNinetyWinBestA', _sci(_fine[0]))
m('PNinetyWinWorstA', _sci(_fine[-1]))
...
# and the column really is the measured trigger ratio times the nominal
# trigger, not a response-corrected power in disguise
from sel_curve import TRIG as _SELTRIG
_rat = sorted(F(r['eirp_p90_W']) / F(r['eirp_nominal_W']) for r in ROWS
              if r['resolution_class'] == 'fine' and F(r['eirp_p90_W'])
              and F(r['eirp_nominal_W']))
assert abs(_rat[0] / _SELTRIG['fine'] - 1) < 1e-5 and \
    abs(_rat[-1] / _SELTRIG['fine'] - 1) < 1e-5, (
        'the catalogue P90 column is not the measured trigger ratio: '
        '%.4f--%.4f against %.4f' % (_rat[0], _rat[-1], _SELTRIG['fine']))
...
# v3.99 post-push fix: the assertion that used to stand here was
...
_p90all = sorted(F(r['eirp_p90_W']) for r in ROWS if F(r['eirp_p90_W']))
m('EirpNinetyMedian', _sci(_p90all[len(_p90all) // 2]))
...
# R2-S1: the per-class mask cost used to be computed here from _LAB, a
# stale list of 8 transitions, while the adopted mask holds 17 transitions
# of 11 species at database precision. The two are not the same mask, which
# is why 0.9 per cent of the Class A union and 0.3 per cent of Class B could
# not be reconciled with 4.1 per cent of the total union. It is now computed
# in v361_calc.py, which holds the adopted list.
...
# Publish the host list so that no OTHER generator has to hard-code it.
# hosts_v399.py carried a literal list of four hosts written before the
# ACA control geometry was repaired; two of those four were artefacts of
# the defect and the table went on naming them. A generator that names
# results must read them.
json.dump(sorted({r['star_name'] for r in _unatt}),
          open('unattributed_hosts.json', 'w'), indent=1)
...
# v4.00: the measured end-to-end trigger completeness, not the superseded
# post-baseline campaign's factor.
m('PNinetyOverTrig', '%.1f' % _SELTRIG['fine'])
