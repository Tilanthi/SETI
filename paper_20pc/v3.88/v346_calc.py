...
Inputs, all local to this folder:
  ...
  archive_meta_v381.json       array and antenna count per execution block
...
AMETA = json.load(open('archive_meta_v381.json'))
...
# pipeline's own result files (bpic_epochs_v347.json, scalars only)
BPIC = json.load(open('bpic_epochs_v347.json'))['products']
...
from bpic_vsys import VSYS_BPIC_KMS as VSYS     # single-sourced; see bpic_vsys.py
try:
