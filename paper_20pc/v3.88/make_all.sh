...
python3 stageonenull_v388.py        > /dev/null   # -> survey_numbers_round47.tex (R1-4: the null for UNATTRIBUTED STAGE-1 events, trigger condition included)
python3 falsealarm_v388.py          > /dev/null   # -> survey_numbers_round46.tex, tab_falsealarm_v388.tex (R1-1); must follow stageonenull
python3 occurrence_v388.py          > /dev/null   # -> survey_numbers_round48.tex (R2-2: CWTFM / Transmitter Rate / hosting fraction); cross-asserts NSysClassA, UnionClassA, SysUnionAMed
python3 rfi_v388.py                 > /dev/null   # -> survey_numbers_round49.tex (R2-3: RFI vetting of the unattributed events)
python3 extension_v388.py           > /dev/null   # -> survey_numbers_round50.tex + tab_extension_v388.tex (v3.88: the PROSPECTIVE EPOCH EXTENSION; reads export_extension.json, applies the survey's eps Eri B6 withholding rule)
