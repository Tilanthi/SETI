...
assert all(r['fs_ratio'] for r in GOOD), 'window with no archive resolution'
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.52.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('catalogue -> per_target_results_v3.52.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
