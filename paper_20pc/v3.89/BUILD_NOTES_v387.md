...
`CHECKLIST_V389.md`. Response letter: `REFEREE_RESPONSE_V389.md`. Every
...
`stageonenull_v389.py` imposes both conditions. The per-window
...
`occurrence_v389.py`, added for R2-2, wrote the file
`stageonenull_v389.py` owns. The build reported 0 errors and 0 undefined
...
v3.89 generators look their inputs up by name in the round files
...
| file | purpose |
|---|---|
| `stageonenull_v389.py` | the correctly-conditioned stage-1 null (R1-1, R1-4) → round 47 |
| `falsealarm_v389.py` | one table of all seven expectations, with `predicts` (R1-1) → round 46 |
| `occurrence_v389.py` | CWTFM, Transmitter Rate, hosting fraction (R2-2) → round 48 |
| `rfi_v389.py` | three RFI vetting measurements (R2-3) → round 49 |
...
`occurrence_v389.py` cross-asserts its inputs against `NSysClassA`,
...
Unchanged from v3.86, plus: `stageonenull_v389.py` **before**
`falsealarm_v389.py` (which reads its macros), and both before
`stagetable_v386.py`. `occurrence_v389.py` and `rfi_v389.py` read only the
...
literal, v3.62 printed frozen-input arithmetic, and v3.89 printed a ledger
