#!/usr/bin/env python3
r"""v3.44 generator: control-ring geometry, repaired line mask, sensitivity
...
CAT = {
    ...
    '[CI](1-0)':  492.1606510,                    # new in v3.44
    'H30a':       231.9009280,                    # new in v3.44
}
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.44.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('catalogue -> per_target_results_v3.44.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
