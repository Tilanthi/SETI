...
PAIRS=[('2MASS J05241914-1601153 551040','2MASS J05241914-1601153 717696'),
       ...
       # v3.44 (referee A1): HD 139084B appears under two catalogue entries with
...
for P in (3e13,1e14,1e15,1e16,1e17):
    unit=[];uni=[];fine=[];allw=[];unit5=[];fine5=[];all5=[];all1=[];unit1=[];fine1=[];unit01=[];fine01=[]
    nsys=nfin=nall=0
    for s_,rs in bysys.items():
        ...
        d5=1.0-(1.0-0.5)**ni; d1=1.0-(1.0-0.1)**ni; d01=1.0-(1.0-0.01)**ni
        ...
        unit1.append(d1*a); fine1.append(d1*f2)
        unit01.append(d01*a); fine01.append(d01*f2)
    S['occurrence']['%.0e'%P]={'n_sys':nsys,'n_sys_fine':nfin,'n_sys_all':nall,
        ...
        'unit_D01':limit(unit1),'measured_D01':limit(fine1),
        'unit_D001':limit(unit01),'measured_D001':limit(fine01)}
# multi-epoch systems, for the epoch-activity ladder (was read from the stale
# duty_v331.json, which is frozen at N_sys=82; single-sourced here in v3.44)
S['n_multi']=sum(1 for s_,rs in bysys.items() if len({r['eb'] for r in rs})>1)
S['n_sys_total']=len(bysys)
