...
\NSystems{} systems within 40\,pc for unresolved spectral carriers, comparing each stellar position with \NCtrl{} controls in an annulus. \textbf{No credible technosignature is found.} The sample is every star within 40\,pc meeting our public-archive
...
M dwarfs. Barnard's Star and Wolf~359 gain first millimetre limits. Four windows raise a stage-1 spatial outlier. Two toward $\beta$~Pic and one toward HD~48370 match
...
archival block at the same tuning. It fails the pre-defined recurrence
criterion and is retired as a candidate, though the data cannot separate a
statistical or instrumental excursion from a non-repeating event.
...
windows and $\EirpMinB$--$\EirpMaxB$\,W over the \NWinB{} coarse ones, against
...
polarisation-blind, and the drift ceiling does not fully cover the
shortest-period known planets, including TRAPPIST-1\,b. The statistic and line mask were defined on these data, so the rank and
...
\S\ref{sec:dwell}. A continuous carrier is the best case and a short-duty
...
boundaries. No ancillary analysis carries a technosignature disposition. The
...
the dense forest of molecular lines raises the confusion background. Our pilot \citep[][citation provisional]{White2026} extended that
...
of view of at least one public ALMA execution block. The star need not sit at
...
volume, a mismatch that bears on astrobiological priority while leaving
statistical validity intact. \NExoHosts{} of the \NStars{} processed stars are confirmed exoplanet hosts
...
sample requires $\sigma_\varpi/\varpi\le0.8$ per cent and the reference
census $\le$10 per cent (Table~\ref{tab:selfunc}), so the ratio compares two
differently selected populations. The census is also incomplete at both ends,
...
\NSystems{} systems, the paired rows being a bookkeeping convenience with no
bearing on the number of independent trials. The \NWindows{} rows comprise
...
\S\ref{sec:technosearch}. They are targeted follow-ups and no part of the
survey, so the counts in this section and the trials budget of
\S\ref{sec:technosearch} are unchanged by them. The unsearched epochs
therefore carry usable information, and some of it has now been used.
...
ALMA target lists, which differs in degree from an interest-selected sample;
here that degree is quantified, at a bycatch fraction of 1 star in 44 and 1 EB
in 57.
...
trial drift the stack evaluates, and no prior on transmitter
...
measurement set as delivered, working per integration and per native channel,
carrying the archive's QA2 flagging through untouched and applying no
...
quantities consumed and emitted per window. For a sky position $\mathbf{x}$, each per-integration
...
in time, and shared by the star and by every control. The across-position median
...
\RingGainOutTrue). Star and controls are therefore not exchangeable in flux
density, while they remain exchangeable in the signal-to-noise ratio the
statistic actually ranks, because primary-beam correction divides an extracted
amplitude and its noise by the same $G(r)$ and leaves $T(\mathbf{x})$ in
Eq.~\ref{eq:tstar} invariant under it. Extended sky brightness does not cancel,
...
Reproducing CP$-$72~2713's \CpTwoFluxMjy\,mJy feature needs
...
and no achievable ALMA sensitivity.
...
systematic within a valid beam model. Excluding Sirius~B would change neither endpoint of the quoted EIRP
...
Class~A threshold $\EirpMedA$\,W. Neither benchmark implies a transmitter model.
...
contains the star. Their separations from the nearest field centre run
from \SepMinArc{} arcsec to \SepMaxArc{} arcsec, with a median of
\SepMedianDeg{} degrees, because the observations were pointed at
solar-system targets, chiefly the Sun, Venus, Neptune, Uranus, several
comets and the icy moons. Such deliveries carry a very large quoted
field of view, reaching \FovMaxDeg{} degrees of half-width for the Sun,
and a candidate crossmatch that trusts that quantity will admit stars
that lie far outside any real primary beam. Proper motion cannot close
the gap: the fastest star in this set moves \PmFastest{} arcsec per
year, so even the smallest separation would take about \PmYears{} years
to remove. The remaining \NPointed{} entries are genuine processing
...
falling outside Bands~3 to~8 and outside this release. The lesson for archival surveys is practical: a positional
...
here is \NStars{} of about \NGenuinelyCovered{} genuinely covered stars rather
...
Band~7, all at \DefectChanwMHz\,MHz, spread over \NDefectEBs{} execution blocks
in \NDefectProjects{} projects; and every one has a finer-channelised window in
the same execution block, covering the same frequencies, that behaves normally
(Appendix~\ref{app:exclusions}). That is the pattern of an auxiliary window
riding on a science baseband with an amplitude scale that never reached the
calibrated one. The screen's reach can be bounded directly as well. Predicting $q$ for every retained window from its own band, array and
antenna count, the residuals are unimodal about their median, with
\SefdResidPctHalfDex{} per cent inside half a decade of it, an interquantile
spread of a factor \SefdResidSpreadFac{} and no empty interval wider than
\SefdResidGapDex\,dex inside the distribution; the excluded six lie
\SefdResidDefGapDex\,dex below the lowest retained window with nothing between.
There is no continuum of smaller deflations hiding under the cut.
...
same $\tau$~Ceti Band~6 window differ by 44 per cent in threshold, and the two
HR~1010 Band~6 rows straddle the $5\sigma$ threshold at 5.10 and 4.95, so the
crossing count is rule-dependent at the level of one window. The four flagged
windows do not depend on the rule, since no discarded row is both a crossing
...
control-position null, and no Gaussian-tail $p$-value. Under exchangeability
...
\BpicCoRingRank th largest control maximum in the survey, below HD~48370's
...
in one 122-kHz channel with the ring at the same level, which is a bright line
filling the beam. The $^{13}$CO($2{\to}1$) window of the same execution block has a
...
emission is resolved away from the star by a beam or two and clustered
off-centre. A third window is consistent: all twelve Band~8 windows here belong to
...
\NCtrl{} controls can express, so the margin records a resolution floor and no
...
frame. Width is consistent with that reading, since in the de-drifted plane the
feature occupies one \CpTwoChanwkHz\,kHz channel, \CpTwoDv\,km\,s$^{-1}$, against
...
\MaskNSpecNew-species mask is a contamination screen and no disposition tool.
...
rms gives $3\sigma$ integrated CO($2{\to}1$) below
...
The observing unit behind the Band~3 flag holds further public blocks that the
download budget of \S\ref{sec:sample} declined, and \BpRecThreeNBlocksExtra{}
have since been calibrated and searched unmodified. The CO($1\rightarrow0$)
feature is in all \BpRecThreeNBlocks, at $T_\star=\BpRecThreeTList$, with
\BpRecThreeHitsList{} channels above threshold and no control of the \NCtrl{}
reaching the star in any block. A second Band~6 CO($2\rightarrow1$) window,
\FobsBpicSixB\,GHz in a different unit, crosses the threshold without becoming
a stage-1 spatial outlier and behaves the same way across its two blocks:
...
$\BpRecSixFluxTwo\pm\BpRecSixErrTwo$\,mJy\,MHz, a difference of
\BpRecSixFluxSig$\sigma$. The stage-1 Band~6 window at \FobsBpicSix\,GHz has no
second block in the archive and is not part of this test.

Frequencies are compared after removing each block's own tuning, which ALMA
sets per date and which therefore absorbs most of the barycentric difference;
on that basis the Band~3 peaks agree to \BpRecThreeTuneChanMax{} channel and the
Band~6 peaks to \BpRecSixTuneChanMax. In the barycentric frame the CO peaks of
the \BpRecNBlocksBary{} blocks with a recorded epoch span
\BpRecVsysList\,km\,s$^{-1}$, within \BpRecVsysSpread\,km\,s$^{-1}$ of
$\beta$~Pictoris' systemic velocity across two transitions and nine years. Real
emission repeats at a fixed frequency and strength on this pipeline while the
CP$-$72~2713 feature does not repeat at all, so its non-recurrence is a property
of the source and no failure of sensitivity.
...
independently shows the same CO component.}
\end{table*}
...
The EIRP$_{5\sigma}$ values are nominal trigger powers (boxed rule,
...
$\CpRecFluxOne\pm\CpRecErrOne$\,mJy. It fails the recurrence criterion pinned
...
The continuum lane is an exploratory by-product and no co-equal search, with
...
of a small sample and no significant departure), consistent with a
...
ensemble, and no part of this budget. Beyond the shared primary-beam gain, neighbouring
...
Table~\ref{tab:searchspace}, and it is kept for the framework, whatever number it returns. A zero detection constrains a fraction of systems only through a
