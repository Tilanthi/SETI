...
M dwarfs. Barnard's Star and Wolf~359 gain first millimetre limits. Four windows are stage-1 spatial outliers. Two toward $\beta$~Pic and one toward HD~48370 match
...
this work. The three contributions listed in \S\ref{sec:intro} are new, to our
...
does not imply greater sensitivity to a hertz-wide carrier. Thresholds are nominal (boxed rule, \S\ref{sec:method});
...
science baseband looks like. The screen's reach is bounded directly as well.
Predicting $q$ for every retained window from its own band, array and antenna
count, the residuals are unimodal, \SefdResidPctHalfDex{} per cent inside half a
decade of the median, with an interquantile spread of a factor
\SefdResidSpreadFac{} and no internal gap wider than \SefdResidGapDex\,dex; the
excluded six lie \SefdResidDefGapDex\,dex below the lowest retained window with
nothing between, so no continuum of smaller deflations hides under the cut.
...
download budget of \S\ref{sec:sample} declined, and \BpRecThreeNBlocksExtra{} of
them have since been calibrated and searched unmodified. The CO($1\rightarrow0$)
feature is present in all \BpRecThreeNBlocks{} blocks, at
$T_\star=\BpRecThreeTList$, with
...
Band~6 peaks to \BpRecSixTuneChanMax{} channels. In the barycentric frame the CO peaks of
...
searched-and-empty. A deliberate transmitter could of course sit on a
molecular transition, so those channels are searched but intrinsically
ambiguous without a further discriminant.
...
Table~\ref{tab:searchspace}; the appendix is kept for the framework, whatever
number it returns. A zero detection constrains a fraction of systems only
through a per-system detection probability, factorised as
%
\begin{equation}
