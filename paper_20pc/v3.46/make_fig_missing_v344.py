...
shipped here:
  ...
  Fig. 5  <- localnull_code/ctrlmax.json   (the window's 512 control statistics)
            per_target_results_v3.46.csv   (T_star and the ring maximum)
  Fig. 7  <- per_target_results_v3.46.csv  (window centre, nominal EIRP, class)
...
ROWS = list(csv.DictReader(open(os.path.join(HERE, "per_target_results_v3.46.csv"))))
...
def fig_cp72_controls():
    ...
    rec = json.load(open(os.path.join(HERE, "cp72_recurrence_v345.json")))
