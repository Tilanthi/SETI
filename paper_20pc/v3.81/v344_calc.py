...
ROWS = list(csv.DictReader(open(HERE + '/per_target_results_v3.81.csv')))
...
NSRCH = len(_cat_ebs)
_all_proc = sum(len(v['searched']) for v in MOUS.values())
M('NEbProcessedAll', '%d' % _all_proc)
M('NEbHeldOut', '%d' % (_all_proc - NSRCH))
assert _all_proc >= NSRCH, (_all_proc, NSRCH)
