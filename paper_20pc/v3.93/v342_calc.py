...
SRC = 'frozen_export_v3.81_survey.json'
...
from inject_curve import P90_OVER_TRIG, TRANSFER_P90
P90_LO, P90_HI = TRANSFER_P90
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'eirp_p90_W', 'eirp_p90_lo_W', 'eirp_p90_hi_W',
        ...
        'disposition']
with open('per_target_results_v3.93.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
    for r in sorted(GOOD, key=lambda r: (r['dist_pc'], r['star_name'], r['band_x'],
        ...
        w.writerow([
            ...
            '%.6e' % (r['eirp_eff_total'] * P90_OVER_TRIG),
            '%.6e' % (r['eirp_eff_total'] * P90_OVER_TRIG * P90_LO),
            '%.6e' % (r['eirp_eff_total'] * P90_OVER_TRIG * P90_HI),
...
with open('per_target_results_v3.93.csv') as _fh:
...
print('catalogue -> per_target_results_v3.93.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
