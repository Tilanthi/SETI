# BUILD NOTES — v3.93: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v393.py`, now asserts that
...
to do if it failed. New `radval_v393.py` does exactly that on
