...
V = sys.argv[1] if len(sys.argv) > 1 else "technosignatures_20pc_v3.42.pdf"
doc = pymupdf.open(V)
TOP, BOT = 43.8, 737.6
H = BOT - TOP
...
pg_ack, y_ack = find(r"ACKNOWLEDGEMENTS.*")
pg_app, y_app = find(r"APPENDIX")
pg_ref, y_ref = find(r"REFERENCES")
N = len(doc)
