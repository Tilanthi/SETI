...
HISTORICAL: this script belongs to the v3.47 -> v3.48 cycle and is kept for the
audit trail.  It operates on technosignatures_20pc_v3.48.tex, which is not in
this folder, so it cannot be run here; against any other file every anchor
fails and nothing is written.  The v3.49 edits are in apply_v349_edits.py.
(v3.49: the emitted macro was NSys, which is defined nowhere; corrected to
NSystems, so the script now reproduces the v3.48 manuscript exactly.)
"""
import io, sys
