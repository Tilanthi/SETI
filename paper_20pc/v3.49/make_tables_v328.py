...
L.append('\\caption{Selection function of the searched sample against a '
         ...
         'at the distances the search itself used. Temperatures are available '
         'for %d of the %d stars (all %d matched to the %d-entry '
         'ALMA-covered census, %d by exact name; %d of those matches '
         'carry a temperature), so the temperature panel is normalised within '
         'each column over classified objects only, with $T_{\\rm eff}$ a '
         'proxy for spectral class. ``Ratio\'\' is the sample column '
         'fraction over the census column fraction: 1 means the '
         'sample tracks the reference population, $>1$ over-representation.}'
