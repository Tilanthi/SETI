...
echo "--- generator/round-file ownership (v3.96: added after a silent overwrite) ---"
python3 roundcollide.py

echo "--- cross-reference resolution (R2-t2) ---"
python3 xrefcheck.py

echo "--- primary-statistic consistency (referee 2, v3.96) ---"
python3 consistency_v396.py

echo "--- macro leakage (referee 2, v3.96) ---"
echo "--- prose literals vs macros (referee 2, M6) ---"
python3 prosenum_v396.py
