# BUILD NOTES — v3.96: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v396.py`, now asserts that
...
to do if it failed. New `radval_v396.py` does exactly that on
