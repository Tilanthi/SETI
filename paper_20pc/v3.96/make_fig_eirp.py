...
import os as _os
_src = open('survey_stats.py').read().split("S=collections.OrderedDict()")[0]
_src = _src.replace("SRC='/workspace/SETI/figwork/v331_data.json'",
...
ax.scatter([r['dist_pc'] for r in coarse],[r['eirp'] for r in coarse],s=13,marker='v',
           ...
           label='deepest window, Class B (%d; completeness unmeasured)'%len(coarse))
ax.scatter([r['dist_pc'] for r in fine],[r['eirp'] for r in fine],s=15,marker='o',
           facecolor='#1f77b4',edgecolor='none',alpha=0.85,
           label='deepest window, Class A (%d; completeness measured)'%len(fine))
...
ax.set_xlabel('distance (pc)'); ax.set_ylabel('EIRP$_{5\\sigma}$ threshold (W)\n(nominal, not completeness-corrected)')
