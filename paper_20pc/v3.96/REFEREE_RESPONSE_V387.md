# Response to the two referee reports — v3.96
...
New `falsealarm_v396.py` builds `tab_falsealarm_v396.tex`
...
New `stageonenull_v396.py` computes the correctly-conditioned null: for
...
Done, computed, not asserted. New `occurrence_v396.py` implements the
...
**1. A silent round-file overwrite.** `occurrence_v396.py`, added for
R2-2, wrote `survey_numbers_round47.tex` — which `stageonenull_v396.py`
...
Three v3.96 generators look their inputs up by name in the round files.
