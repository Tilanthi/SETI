...
# ---------------------------------------------------------------- unit repair
# v3.69 (referee 1, point 5).  The twelve Band 9/10 rows restored in v3.60 were
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.69.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('catalogue -> per_target_results_v3.69.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
