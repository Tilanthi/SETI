...
shipped here:
  ...
  Fig. 5  <- localnull_code/ctrlmax.json   (the window's 512 control statistics)
            per_target_results_v3.97.csv   (T_star and the ring maximum)
  Fig. 7  <- per_target_results_v3.97.csv  (window centre, nominal EIRP, class)
...
def fig_cp72_controls():
    ...
    # Single-column canvas printed at 1:1. The previous 302 pt canvas was
    # printed at 177 pt, scale 0.58, so the type came out at little over
    # half the size it was set in.
    fig = plt.figure(figsize=(245.0 / 72.0, 175.0 / 72.0), layout="constrained")
    ...
    ax.set_xlabel("single-position control statistic", fontsize=7.6)
    ax.set_ylabel("number of control positions", fontsize=7.6)
    ax.tick_params(labelsize=6.8)
    ax.legend(loc="upper right", fontsize=5.9, borderpad=0.3,
              handlelength=1.5, handletextpad=0.5, labelspacing=0.28,
              frameon=False)
...
def fig_sensitivity_2d():
    ...
    import re as _re
    _txt = "".join(open(os.path.join(HERE, _f)).read()
                   for _f in sorted(os.listdir(HERE))
                   if _f.startswith("survey_numbers") and _f.endswith(".tex"))
    _han = float(_re.search(r"\\newcommand\{\\HanFacMed\}\{([0-9.]+)\}",
                            _txt).group(1))
    axr = ax.secondary_yaxis(
        "right", functions=(lambda v: v * _han, lambda v: v / _han))
    # v3.72 (round 2, referee D1): P_eff,total is a PER-WINDOW catalogue column
    # spanning x1.33-3.98; 2.29 is only its median.  Label the axis as the
    # median scaling it is, and print the range, so no reader takes a specific
    # window's effective threshold off a single-factor axis.
    # The per-window range is printed in the LaTeX caption, not on the axis:
    # a two-line ylabel overran the figure bbox and was silently clipped
    # (round 3, referee I1; same failure as v3.50).
    _eff = sorted(f(r, "eirp_eff_total_W") / f(r, "eirp_nominal_W")
                  for r in ROWS if f(r, "eirp_nominal_W") > 0)
    open(os.path.join(HERE, "peff_range_v372.tex"), "w").write(
