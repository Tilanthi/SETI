# BUILD NOTES — v3.97: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v397.py`, now asserts that
...
to do if it failed. New `radval_v397.py` does exactly that on
