...
(Figure~\ref{fig:eirp}, right panel). The non-detections reach 5$\sigma$
upper limits of 0.19--3.6\,mJy (median 0.59\,mJy). Two of the five
...
densities (right; downward triangles are $5\sigma$ upper limits, filled
circles are detections) as a function of distance, for the 19
target/bands searched to date. The single automatically-flagged
candidate ($\beta$~Pictoris, Band 3) is marked with a star and is
identified in the text as CO(1--0) line contamination rather than a
technosignature.}
\label{fig:eirp}
\end{figure*}
...
frequency range actually searched (not the full ALMA tuning -- only the
single finest-resolution spectral window is searched, per
\S\ref{sec:method}), the resulting EIRP$_{\rm min}$ limit, whether the
automated pipeline flagged a credible candidate, and the continuum flux
density (or, for non-detections, the $5\sigma$ upper limit, marked UL).
This table will grow as the survey proceeds; the version current at the
time of writing is reproduced here so that the summary statistics quoted
in the main text are fully traceable.

\begin{table*}
\centering
\footnotesize
\caption{Per-target/band results, ordered by distance. Frequency range is
in GHz; EIRP$_{\rm min}$ is in W. ``Cand.'' indicates whether the
automated pipeline flagged the narrowband search as a credible
technosignature candidate (Y/N); see \S\ref{sec:results} for the
disposition of the one Y in this table.}
\label{tab:full}
\begin{tabularx}{\textwidth}{@{}l c c c c c c X@{}}
\hline
Target & Band & Dist.\ (pc) & Freq.\ range (GHz) & EIRP$_{\rm min}$ (W) & Cand.\ & Continuum (mJy) & Notes \\
\hline
Proxima Cen & B6 & 1.30 & 223.09--224.91 & 3.43e+13 & N & 0.666 (UL) &  \\
NAME Barnard's star & B7 & 1.83 & \ldots & \ldots & \ldots & 3.622 (UL) & narrowband search failed \\
Wolf  359 & B7 & 2.41 & \ldots & \ldots & \ldots & 3.380 (UL) & narrowband search failed \\
