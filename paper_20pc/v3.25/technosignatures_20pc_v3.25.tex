...
\journalinfo{Paper version 3.25 (2026 September 9). Sample rescoped
from 20\,pc to the full current search volume (40\,pc). Live snapshot
2026 September 9, 15:58\,UT: 114 of 208 planned target/bands (55\%) across
86 of the 168 census stars, 448 spectral windows. The candidate criterion
is now the symmetric single-position star-versus-control test; see the
version-control changelog in the Data Availability section.}
...
across 86 of the 168 census stars and 448 spectral windows, spanning
...
Nominal 5$\sigma$ thresholds span $2.0\times10^{12}$--$2.8\times
10^{17}$\,W in EIRP (median $1.0\times10^{15}$\,W); because most windows are channelised at
...
criteria. This identifies 168 unique target stars, processed
...
The detailed results that follow report the $\le$20\,pc release of
version 3.24 (see Scope of this version). Of the 148 target/bands
planned for that subsample (spanning 46 unique stars), 60 were searched to
...
Status of the 148 target/bands planned for the $\le$20\,pc release: 60 searched to completion and
...
versions maximised the on-source quantity over a small region centred
on that position while each control remained a single position, and
that asymmetry was not benign. Under a pure-noise null with
exchangeable positions, the maximum of $n_{\rm src}$ source-region
positions exceeds the maximum of $n_{\rm ctrl}$ single controls with
probability $n_{\rm src}/(n_{\rm src}+n_{\rm ctrl})$; for the
$n_{\rm src}=135$ region and 512 controls of a representative window
that is 21 per cent, and the measured region-max star-beats-ring rate
across this snapshot, 67 of 448 windows (15.0 per cent), is of that
order. The symmetric statistic instead produces star-beats-ring in 6
of 448 windows (1.3 per cent), consistent with the exchangeable
expectation $1/513=0.19$ per cent per window inflated by the real
maps' heavy tails, and stable against the 1.3 per cent measured on the
$\le$20\,pc subsample alone. Adopting it makes the nominal
exchangeability rate the operative one, so the survey-level
false-alarm expectation follows directly rather than through the
bookkeeping ladder that the asymmetry previously required.
...
crossing at the stellar position. With a symmetric statistic the
survey-level expectation is immediate: each window contributes at most
the exchangeable rate $1/(1+n_{\rm ctrl})=1/513$, so across 448
windows the expected number of flags under the pure-noise null is
$448/513=0.87$. Four flagged target/bands against 0.87 expected is not
a significant excess, and three of the four carry independent
astrophysical attributions (below), leaving one marginal crossing --
precisely the order of the expectation. This is the
...
A further limit of the ensemble deserves statement: with 512
controls the smallest resolvable per-window $p$-value is $1/513$, so a
Bonferroni threshold over 448 windows ($1.1\times10^{-4}$) lies below
what the control ensemble can measure. The controls can therefore
establish that a crossing is unremarkable, but cannot on their own
certify one as significant survey-wide; that burden falls on
localisation, recurrence, and astrophysical attribution, which is how
each flag below is decided. The two $\beta$~Pictoris dispositions do not depend on the
...
candidate through version 3.24 and is \emph{not} a candidate under the
symmetric criterion adopted here. It is a single-epoch threshold
...
statistic, an empirical $p=0.021$ that is nowhere near the $1/513$
floor the criterion requires. It also fails two direct and pre-named
tests -- it is not localised at the stellar position, and it does not
recur at a second epoch (Table~\ref{tab:aumic}). It is the worked
example that motivated the change: under the old region-max statistic
its 135-position source maximum (5.97) cleared the control maximum
(5.85) while the star itself (5.22) did not, which is exactly the
asymmetry quantified in \S\ref{sec:technosearch}.
...
% `submitted-v3.25` on the SETI repository commit that corresponds to the
...
the repository commit carrying the tag \texttt{submitted-v3.25},
