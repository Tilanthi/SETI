...
def fig_eirp_context(good, outdir, report):
    ...
    fig = plt.figure(figsize=(3.4, 4.35), layout="constrained")
    ...
    ax.text(4.5, ARECIBO_W / 1.28, "Arecibo-like planetary radar",
            fontsize=5.2, color="#009E73", ha="left", va="top", zorder=6,
    ...
    leg = ax.legend(handles, labels, loc="lower right", fontsize=5.2,
    ...
    axb.text(1.05, 8.0e6,
             ...
             % len(good),
             fontsize=5.2, color="0.25", ha="left", va="center", zorder=7)

    # resolution-span bracket, far right, in the empty x > 500 GHz margin
    axb.annotate("", xy=(600.0, 3.2e7), xytext=(600.0, 3.0),
                 arrowprops=dict(arrowstyle="<->", lw=0.7, color="0.35"))
    axb.text(430.0, 3.0e3, r"$\Delta\log\Delta\nu_{\rm ch}\!\approx\!7$",
             fontsize=5.2, color="0.35", ha="center", va="center",
