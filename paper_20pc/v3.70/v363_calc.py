...
META = json.load(open('archive_meta_v343.json'))['ebs']
NWIN = len(ROWS)
assert NWIN == 443, NWIN
...
M('PbFloorResp', '0.5')            # response floor adopted in v3.70
...
assert len(_bp6) == 1, len(_bp6)
_bp6 = _bp6[0]
_tr = sorted(float(r['n_drift_trials']) for r in ROWS)
assert float(_bp6['n_drift_trials']) == _tr[-1], 'no longer the survey maximum'
assert float(_bp6['eta_drift']) == max(float(r['eta_drift']) for r in ROWS)
...
# interval is an order of magnitude wider.  `tailboot_v370.py` recomputes the
...
_TB = json.load(open('tailboot_v370.json'))
...
assert all(r['search_class'] == 'A' for r in _cross), 'a crossing is not Class A'
M('NCrossClassA', '%d' % len(_cross))
...
# stated in the text.
_CM = json.load(open('campaign_v370.json'))
...
M('CampUnOneStar', _u[0]['star'])
M('CampUnOneBand', '%d' % _u[0]['band'])
M('CampUnOneGHz', '%.3f' % _u[0]['flo'])
M('CampUnOneT', '%.2f' % _u[0]['t'])
M('CampUnOneCtrl', '%.2f' % _u[0]['ctrl'])
M('CampUnOneRepT', '%.2f' % _u[0]['repeat_best_t'])
M('CampUnOneRepNge', '%d' % _u[0]['repeat_nge'])
M('CampUnTwoStar', _u[1]['star'])
M('CampUnTwoT', '%.2f' % _u[1]['t'])
M('CampUnTwoRepT', '%.2f' % _u[1]['repeat_best_t'])
M('CampAsOf', '2026 September 13')
...
# have now been searched.  `searched_ebs_v370.txt` is the list of execution
# blocks carrying a search product on the host, harvested read-only.
_all = {l.strip() for l in open('searched_ebs_v370.txt') if l.strip()}
_frozen = {r['eb'] for r in ROWS}
_meta = json.load(open('archive_meta_v343.json'))['mous']
_prog = set()
for _v in _meta.values():
    _prog |= {p if isinstance(p, str) else p.get('eb') for p in _v['progenitors']}
_b9 = json.load(open('b910_mous_v362.json'))
_prog |= {e.replace('uid://', '').replace('/', '_') for e, _ in _b9['ebs']}
_prog = {p for p in _prog if p}
assert len(_prog) == int(texval('NProgenitorEBTotal')), (len(_prog), texval('NProgenitorEBTotal'))
_camp = _all - _frozen
_in = _frozen | (_camp & _prog)
