...
# M-dwarf row out of tab_selection.tex (v3.70, referee 2 M1).
...
python3 tailboot_v370.py            > /dev/null   # -> tailboot_v370.json (clustered tail CI)
...
python3 audit_numbers_v370.py       > /dev/null   # forensic number audit; fails the build if any number is stale
