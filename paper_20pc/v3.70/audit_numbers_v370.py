...
Exit status is non-zero if any check fails.  Writes `audit_v370.json`.
"""
...
MAC = {}
for fn in sorted(glob.glob('survey_numbers*.tex')) + ['peff_range_v370.tex',
                                                      'regen_count.tex']:
...
R = list(csv.DictReader(open('per_target_results_v3.70.csv')))
...
chk('NCrossClassA == NHits', M('NCrossClassA'), M('NHits'))
...
if M('SearchedUnionGHz') and M('MaskUnionGHz'):
    chk('gross - masked == unmasked', '%.2f' % (union - num(M('MaskUnionGHz'))),
        '%.2f' % num(M('SearchedUnionGHz')), tol=0.06)
...
CM = json.load(open('campaign_v370.json'))
...
ALL = {l.strip() for l in open('searched_ebs_v370.txt') if l.strip()}
frozen = {r['eb'] for r in R}
meta = json.load(open('archive_meta_v343.json'))['mous']
prog = set()
for v in meta.values():
    prog |= {p if isinstance(p, str) else p.get('eb') for p in v['progenitors']}
b9 = json.load(open('b910_mous_v362.json'))
prog |= {e.replace('uid://', '').replace('/', '_') for e, _ in b9['ebs']}
...
inscope = frozen | ((ALL - frozen) & prog)
...
# tail factor
TB = json.load(open('tailboot_v370.json'))
...
try:
    ...
    figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}',
                                 open('technosignatures_40pc_v3.70.tex').read())))
    figreport = {}
    for g in figs:
    ...
    json.dump(figreport, open('audit_figures_v370.json', 'w'), indent=1)
except ImportError:
...
json.dump({'pass': len(OK), 'fail': [list(map(str, x)) for x in FAIL]},
          open('audit_v370.json', 'w'), indent=1)
