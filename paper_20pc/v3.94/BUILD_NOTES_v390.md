# BUILD NOTES — v3.94: the radius correction fails its own validation
...
Both are fixed, and a new gate, `consistency_v394.py`, now asserts that
...
to do if it failed. New `radval_v394.py` does exactly that on
