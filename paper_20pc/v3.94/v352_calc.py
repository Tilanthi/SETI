...
Inputs (all frozen; all in this directory unless noted):
  frozen_export_v3.81_survey.json        the per-window search export (512 controls)
  per_target_results_v3.94.csv    the released catalogue (431 windows)
...
F = float
ROWS = list(csv.DictReader(open(os.path.join(HERE, 'per_target_results_v3.94.csv'))))
...
def load_windows():
    ...
    d = json.load(open(os.path.join(HERE, 'frozen_export_v3.81_survey.json')))
    rows = d['rows']
...
M('HanFacAvgMedFour', '%.2f' % peak_factor_median(N_AVG_LO))
M('HanFacAvgMedTwo', '%.2f' % peak_factor_median(N_AVG_HI))
...
CATALOGUE_FILE = 'per_target_results_v3.94.csv'
