...
DEFAULT_DATA = os.path.join(HERE, "frozen_export_v3.81_survey.json")  # v3.46: local copy;
...
def _eff_index():
    """Per-window effective power, keyed (eb, lower edge), from the
    released catalogue: the frozen export carries only the trigger."""
    import csv as _csv
    idx = {}
    for fn in ("per_target_results_v3.94.csv",):
...
def p90_of(r):
    """Per-window 90 per cent recovery power."""
    eff = EFF.get((r["eb"], round(min(r["flo"], r["fhi"]), 4)))
    if eff is None:
...
def fig_eirp_context(good, outdir, report):
    ...
    # R1-3: name the quantity exactly. This panel plots the TRIGGER-level
    # P_90, not the headline P_90^sel, because the literature points are
    # threshold statistics; P_90^sel is higher still, so the panel
    # flatters this survey and must say which number it shows.
    ax.set_ylabel("trigger-level $P_{90}$, this survey (W)\n"
                  "literature points are each programme's own 5$\\sigma$:\n"
                  "NOT a like-for-like sensitivity axis", labelpad=1.5)
    ...
    axb.text(0.025, 0.035,
             "Depths are NOT comparable between\nprogrammes: channel widths "
    ...
    # R1-9 (v3.94): the panels are also written separately, so the main
