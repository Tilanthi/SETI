...
D = sys.argv[1]
src = os.getcwd()
tex = open(src + '/technosignatures_40pc_v3.68.tex').read()
inputs = sorted(set(re.findall(r'\\input\{([^}]*)\}', tex)))
figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}', tex)))
files = ['technosignatures_40pc_v3.68.tex', 'openjournal.cls', 'epsf.sty'] + [i + '.tex' for i in inputs]
for f in files:
...
PY
cd "$D"
for i in 1 2 3; do pdflatex -interaction=nonstopmode technosignatures_40pc_v3.68.tex > /dev/null 2>&1; done
python3 - <<'PY'
import re, pymupdf
log = open('technosignatures_40pc_v3.68.log', errors='ignore').read()
...
d = pymupdf.open('technosignatures_40pc_v3.68.pdf')
