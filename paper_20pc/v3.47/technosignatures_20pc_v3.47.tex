...
TRAPPIST-1\,b, the only sample planet to reach it. The statistic and line mask were defined on these data, so the rank and
...
width $\Delta\nu_{\rm ch}$. The track is the right time-scale, the
...
and the two are not interchangeable. A window is \emph{drift-resolving} when
...
emission can be circumstellar; and the sight line runs through the outer
...
into its own window. $T_\star$ exceeds the ring maximum by \CpTwoMargin, the smallest margin
...
Band~6 window and is released with the catalogue. A split-half of
...
independent epochs. The unit behind the Band~3 flag holds
...
$\BpRecSixFluxSig\sigma$. It is Class~\BpRecSixClass{} at
\BpRecSixChanwKHz\,kHz, so the recurrence is demonstrated at the survey's
finest drift discrimination. The stage-1 Band~6 window at
...
star. A transmitter sits at the star; CO in an edge-on belt sits at the ansae,
...
where a carrier occupies one channel. The $\beta$~Pictoris test establishes
...
$\beta$~Pictoris we adopt the gas-derived
...
\texttt{per\_target\_results\_v3.47.csv}, carries \NCatRows{} rows, one per searched window, in \NCatCols{}
...
confined to adjacent channels, the lag-two term being a sixth. Under the $0.25/0.5/0.25$ kernel a
...
\CtrlMedCoarse{} against \FaPredCoarse. That agreement is partly bookkeeping:
