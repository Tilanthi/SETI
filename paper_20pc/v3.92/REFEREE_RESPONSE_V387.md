# Response to the two referee reports — v3.92
...
New `falsealarm_v392.py` builds `tab_falsealarm_v392.tex`
...
New `stageonenull_v392.py` computes the correctly-conditioned null: for
...
Done, computed, not asserted. New `occurrence_v392.py` implements the
...
**1. A silent round-file overwrite.** `occurrence_v392.py`, added for
R2-2, wrote `survey_numbers_round47.tex` — which `stageonenull_v392.py`
...
Three v3.92 generators look their inputs up by name in the round files.
