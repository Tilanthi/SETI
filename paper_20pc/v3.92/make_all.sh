...
python3 make_fig_classa_sens.py     > /dev/null   # -> figures/classa_sensitivity.pdf, round32
...
python3 stageonenull_v392.py        > /dev/null   # -> survey_numbers_round47.tex (R1-4: the null for UNATTRIBUTED STAGE-1 events, trigger condition included)
python3 falsealarm_v392.py          > /dev/null   # -> survey_numbers_round46.tex, tab_falsealarm_v392.tex (R1-1); must follow stageonenull
python3 occurrence_v392.py          > /dev/null   # -> survey_numbers_round48.tex (R2-2: CWTFM / Transmitter Rate / hosting fraction); cross-asserts NSysClassA, UnionClassA, SysUnionAMed
python3 rfi_v392.py                 > /dev/null   # -> survey_numbers_round49.tex (R2-3: RFI vetting of the unattributed events)
python3 extension_v392.py           > /dev/null   # -> survey_numbers_round50.tex + tab_extension_v392.tex (v3.92: the PROSPECTIVE EPOCH EXTENSION; reads export_extension.json, applies the survey's eps Eri B6 withholding rule)
python3 primary_v392.py             > /dev/null   # -> survey_numbers_round51.tex (v3.92 R1-1/R2-2: RADIUS-CORRECTED statistic as PRIMARY, its own conditioned null, tail-factor uncertainty folded in, frozen statistic retained as robustness)
python3 radval_v392.py              > /dev/null   # -> survey_numbers_round52.tex (v3.92 R1-1: PROSPECTIVE out-of-sample validation of the radius correction. VERDICT: FAIL -- the corrected stellar rank is still non-uniform and the median moves further from 0.5, so the frozen screen stays primary and disposition rests on physics)
python3 freqdef_v392.py             > /dev/null   # -> survey_numbers_round53.tex (R1-4: the three frequency-union quantities, defined once and asserted: dnu_A, dnu_B full, dnu_AuB, dnu_B-only)
python3 visextend_v392.py           > /dev/null   # -> survey_numbers_round54.tex (R1-1: cost of extending the visibility test to every threshold crossing)
python3 epochsplit_v392.py          > /dev/null   # -> survey_numbers_round56.tex (R1-8: repeat blocks vs TEMPORALLY INDEPENDENT epochs; 13 'multi-epoch' systems have all blocks within a day)
...
python3 viscal_v392.py              > /dev/null   # -> survey_numbers_round55.tex (R2-1: EMPIRICAL bound on the visibility-calibration term, previously 'not separately quantified'; folds into the combined budget)   # MUST follow p90_budget: reads round38
