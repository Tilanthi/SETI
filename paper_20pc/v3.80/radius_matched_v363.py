...
Outputs `radius_matched_v363.json`: the flagged set under each screen, the
...
FRZ = json.load(open(os.path.join(HERE, 'frozen_ctrl_v363.json')))
...
# ------------------------------------------------- leave-one-star-out check
# Referee 1, point 2: the profile above is measured on the held-out windows
# and then tested on them, which is training and testing on one sample.  Here
# the profile for each window is refitted with that window's STAR removed, so
# the detrended rank of every held-out window is genuinely out of sample with
# respect to the correction applied to it.
HOSTAR = [HOMETA.get(r["f"], {}).get("star", "?") for r in ho]
stars = sorted(set(HOSTAR))
prof_loo = {}
for s_ in stars:
    sub = [r for r, st_ in zip(ho, HOSTAR) if st_ != s_]
    pr = np.zeros(NBIN)
    for b in range(NBIN):
        sel = BIN == b
        pr[b] = float(np.mean([zof(np.asarray(q['c'], float))[sel].mean() for q in sub]))
    prof_loo[s_] = pr
ho_loo = []
for r, st_ in zip(ho, HOSTAR):
    c = np.asarray(r['c'], float)
    t = float(r['star'])
    sc = scale(c)
    pr = prof_loo[st_]
    cdet = c - sc * pr[BIN]
    ho_loo.append((1 + (cdet >= t - sc * pr[0]).sum()) / (c.size + 1))
ho_loo = np.array(ho_loo)
print('\nleave-one-star-out detrended held-out rank: median %.3f  D %.3f p %.4g'
      % (np.median(ho_loo), *ks_uniform(ho_loo)))
print('   (in-sample detrended was quoted above; %d stars, %d windows)'
      % (len(stars), len(ho_loo)))


json.dump(dict(
    _doc=__doc__.strip(),
    loo=dict(n_stars=len(stars), median=float(np.median(ho_loo)),
             D=ks_uniform(ho_loo)[0], p=ks_uniform(ho_loo)[1]),
    ...
    survey=dict(raw_median=float(np.median(surv_raw)),
                ...
                n=len(surv_b)),
), open(os.path.join(HERE, 'radius_matched_v363.json'), 'w'), indent=1)
print('\nwrote radius_matched_v363.json')
