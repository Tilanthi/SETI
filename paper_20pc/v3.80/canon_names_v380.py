...
Writes frozen_export_v3.80.json and canon_names_v380.json (the audit trail).
"""
import json, re, csv, math, collections

NEW = '/workspace/SETI/figdata_v380.json'
...
OUT = '/workspace/SETI/frozen_export_v3.80.json'
...
json.dump({'snapshot': d['snapshot'], 'rows': outside},
          open('/workspace/SETI/outofsample_v380.json', 'w'))
json.dump({'routes': dict(how),
           'unresolved': dict(unresolved),
           'stars': {k: dict(v) for k, v in sorted(audit.items())}},
          open('/workspace/SETI/canon_names_v380.json', 'w'), indent=1)

print('rows total        : %d' % (len(inside) + len(outside)))
print('  within 40 pc    : %d   -> frozen_export_v3.80.json' % len(inside))
print('  beyond 40 pc    : %d   -> outofsample_v380.json (external null)' % len(outside))
