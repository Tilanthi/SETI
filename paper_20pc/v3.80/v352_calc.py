...
Inputs (all frozen; all in this directory unless noted):
  frozen_export_v3.80.json        the per-window search export (512 controls)
  per_target_results_v3.80.csv    the released catalogue (431 windows)
  archive_meta_v380.json          per-EB array, frequency support
  ...
  technosignatures_40pc_v3.80.tex parsed, read-only, for the transfer bracket
...
F = float
ROWS = list(csv.DictReader(open(os.path.join(HERE, 'per_target_results_v3.80.csv'))))
META = json.load(open(os.path.join(HERE, 'archive_meta_v380.json')))
EBS = META['ebs']
...
def load_windows():
    ...
    d = json.load(open(os.path.join(HERE, 'frozen_export_v3.80.json')))
    rows = d['rows']
...
CATALOGUE_FILE = 'per_target_results_v3.80.csv'
