...
R = list(csv.DictReader(open('per_target_results_v3.80.csv')))
...
ALL = {l.strip() for l in open('searched_ebs_v380.txt') if l.strip()}
frozen = {r['eb'] for r in R}
meta = json.load(open('archive_meta_v380.json'))['mous']
prog = set()
...
try:
    ...
    figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}',
                                 open('technosignatures_40pc_v3.80.tex').read())))
    figreport = {}
    for g in figs:
