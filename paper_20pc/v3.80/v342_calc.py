...
ARCH = json.load(open('archive_meta_v380.json'))
EBS = ARCH['ebs']
...
# completed sweep (archive_meta_v380.json), resolving 132 through obscore's
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.80.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('catalogue -> per_target_results_v3.80.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
