...
python3 v380_calc.py                > /dev/null   # -> survey_numbers_round30.tex, tab_flagged_v380.tex
python3 regen_count.py              > /dev/null   # -> regen_count.tex (must be last but one)
python3 audit_numbers_v380.py       > /dev/null   # forensic number audit; fails the build if any number is stale
