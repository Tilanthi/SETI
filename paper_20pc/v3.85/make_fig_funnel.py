...
STEPS = [
    ...
    ("Without an astrophysical attribution (%s by chance)"
     % n("ExpStageOneAll"), n("NStageOneUnattrib"), "events"),
    ("Independently confirmed: recurrence, second epoch, visibilities",
     n("NCandidates"), "candidates"),
]
...
for i, ((cnt, lab), f) in enumerate(zip(FUNNEL, FILL)):
    ...
    axL.text(0.5, (y0 + y1) / 2 - 0.044, lab, ha="center", va="center",
             fontsize=5.8)
...
REGIME = [(0, 3, "0.955", "data"), (3, 6, "0.985", "windows"),
          (6, 10, "0.915", "candidate tests")]
...
for i, (lab, cnt, unit) in enumerate(STEPS):
    ...
    axR.text(0.890, yc, unit, ha="left", va="center", fontsize=5.6,
             color="0.40", zorder=4)
    if i < NS - 1:
        axR.plot([0.055, 0.985], [rtop - (i + 1) * rh] * 2, color="0.80",
                 lw=0.4, zorder=2)
