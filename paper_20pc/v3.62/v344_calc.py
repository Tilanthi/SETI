...
ROWS = list(csv.DictReader(open(HERE + '/per_target_results_v3.62.csv')))
...
#
# v3.62 (referee 1, point 3).  `archive_meta_v343.json` was harvested from the
...
NPROG = sum(v['n_progenitor_ebs'] for v in MOUS.values()) + len(_b910_prog)
NSRCH = sum(len(v['searched']) for v in MOUS.values()) + len(_b910_srch)
_cat_ebs = {r['eb'] for r in ROWS}
assert NSRCH == len(_cat_ebs), (NSRCH, len(_cat_ebs))
