...
for tag, cls in (('A', 'fine'), ('B', 'coarse')):
    ...
    M('EirpHanMin' + tag, sci(e[0] / HAN_BEST))
    M('EirpHanMax' + tag, sci(e[-1] / HAN_WORST))
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.50.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('catalogue -> per_target_results_v3.50.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
