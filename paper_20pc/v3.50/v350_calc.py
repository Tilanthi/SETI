...
Inputs (all frozen, all in this directory):
  ...
  per_target_results_v3.50.csv  the released catalogue (431 windows)
...
# ----------------------------------------------------------------- (2)
HO = json.load(open(os.path.join(HERE, 'heldout_v350.json')))['windows']
# A window carries the beta Pic CO line at the star when the pipeline's own
# nearest-line field is the CO transition of that band AND the window is the
# fine one that resolves it.  Identify them by the criterion the pipeline
# already applied: star statistic above every control (n_control_ge_star = 0)
# or, for the Band 6 recurrence window, a CO detection at the star.
CO_SET = [w for w in HO if w['detection'] and w['nearest_known_line'].startswith('CO')
          and 'fine' in w['resolution_class'] and w['star_peak_snr'] > 5.0]
NOISE = [w for w in HO if w not in CO_SET]

M('HOWindows', '%d' % len(HO))
M('HOBlocks', '%d' % len({w['eb'] for w in HO}))
M('HOStars', '%d' % len({w['star_name'].split(' [')[0] for w in HO}))
M('HOCoWin', '%d' % len(CO_SET))
M('HONoiseWin', '%d' % len(NOISE))
M('HONoiseStageOne', '%d' % sum(1 for w in NOISE if w['n_control_ge_star'] == 0))
M('HOCoTLo', '%.1f' % min(w['star_peak_snr'] for w in CO_SET))
M('HOCoTHi', '%.1f' % max(w['star_peak_snr'] for w in CO_SET))

p_noise = sorted(w['p_star_empirical'] for w in NOISE)
p_all = sorted(w['p_star_empirical'] for w in HO)
M('HONoiseMedP', '%.2f' % st.median(p_noise))
M('HONoiseMinP', '%.2f' % min(p_noise))
M('HONoiseMaxP', '%.2f' % max(p_noise))


def ks_uniform(v):
    """Two-sided one-sample KS against U(0,1).

    scipy's exact small-sample p-value where scipy is available; otherwise
    Stephens' asymptotic form, which on this sample returns p = 0.67 against
    the exact 0.65 and so would change the quoted figure in the second
    decimal.  The generator therefore requires scipy and says so.
    """
    from scipy import stats
    r = stats.kstest(sorted(v), 'uniform')
    return float(r.statistic), float(r.pvalue)


d_n, p_n = ks_uniform(p_noise)
d_a, p_a = ks_uniform(p_all)
M('HOKsD', '%.2f' % d_n)
M('HOKsP', '%.2f' % p_n)
M('HOKsDAll', '%.2f' % d_a)
M('HOKsPAll', '%.2f' % p_a)
M('HONoiseBelowFive', '%d' % sum(1 for x in p_noise if x < 0.05))
M('HONoiseExpFive', '%.1f' % (0.05 * len(p_noise)))
