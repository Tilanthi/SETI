...
'''
import glob
for fn in ['technosignatures_20pc_v3.58.tex']+glob.glob('tab_*.tex'):
    s=open(fn).read()
    ...
    open(fn,'w').write(s)
PY
pdflatex -interaction=nonstopmode technosignatures_20pc_v3.58.tex >/dev/null 2>&1
cd - >/dev/null
python3 - <<'PY'
import re
log=open('/tmp/fm/technosignatures_20pc_v3.58.log',errors='ignore').read()
hts=[float(x) for x in re.findall(r'ASTRAFLOAT ht=([\d.]+)pt',log)]
src=open('technosignatures_20pc_v3.58.tex').read()
app=src.index('\n\\appendix')
ends=[]
