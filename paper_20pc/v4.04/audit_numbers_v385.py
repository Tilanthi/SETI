...
try:
    ...
    figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}',
                                 open('technosignatures_40pc_v4.04.tex').read())))
    figreport = {}
    for g in figs:
...
# (C) FIGURES
# =====================================================================
# The docstring promised this pass from v3.85 and it was never written; the
# file ended with no pdftotext and no figures/ glob.  Implemented in v4.01
# after a referee found HD 14055 and HD 23484 -- two windows the ACA repair
# removed -- still drawn in a figure, which no gate could see.
#
# Two checks per figure, both on text extracted from the built PDF:
#
#   1. the text must be extractable at all.  A matplotlib figure that has
#      not set pdf.fonttype 42 outlines its glyphs, which both breaks this
#      check and puts Type-3 fonts in the manuscript.
#   2. every star designation drawn in the figure must exist in the released
#      catalogue, and must be drawn in the SIMBAD form star_alias.designation
#      produces (R2 S5 typography).
#
# Figures the manuscript does not \includegraphics are reported, never
# failed: they are dead output, and saying so is the point.
import star_alias                                              # noqa: E402

# Two shapes: catalogues whose number follows directly (HD 14055, GJ 3379),
# and the zone catalogues whose designation is a signed zone and a number
# (BD+05 1668, CD-23 12345).  Matching the second with the generic pattern
# yields "BD05", which is not a designation and produced a false failure.
STARPAT = re.compile(
    r'\b(?:BD|CD|CP)\s?[−+-]?\d{1,2}\s?\d{3,5}\b'
    r'|\b(?:HD|HR|HIP|GJ|GL|LP|LHS|TYC|TWA|WD|PM|LSR)\s?\d[\w.+−-]*')
CAT_DESIG = {star_alias.designation(r['star_name'], tex=False) for r in R}
...
_typeset = set()
for _tx in glob.glob('technosignatures_*.tex'):
    _typeset |= {os.path.basename(p) for p in
                 re.findall(r'\\includegraphics[^{]*\{([^}]+)\}',
                            open(_tx, errors='ignore').read())}

for fig in sorted(glob.glob('figures/*.pdf')):
    base = os.path.basename(fig)
    live = base in _typeset
    txt = _fig_text(fig)
    if txt.startswith('\x00') or len(txt.strip()) < 5:
        (FAIL if live else WARN).append(
            ('figure text extractable: ' + base, txt[:60] or 'empty',
             'readable text'))
        continue
    if not live:
        WARN.append(('figure built but never \\includegraphics: ' + base,
                     'dead output', 'in the release deposit'))
    for nm in sorted(set(STARPAT.findall(txt))):
        # a figure renders a typographic minus; the catalogue writes a hyphen
        nm = nm.strip().replace('−', '-')
        canon = star_alias.designation(nm, tex=False)
        if canon not in CAT_DESIG:
            (FAIL if live else WARN).append(
                ('star in %s not in the catalogue' % base, nm, 'a released star'))
        elif canon != nm:
            (FAIL if live else WARN).append(
                ('star in %s not in SIMBAD form' % base, nm, canon))

# ---- the same check on the manuscript and the table fragments ------------
# R2 S5 lists five designations set in a non-SIMBAD form.  They came from
# three different generators, each with its own idea of a printable name.
_inputs = set()
for _tx in glob.glob('technosignatures_*.tex'):
    _body = open(_tx, errors='ignore').read()
    _inputs |= set(re.findall(r'\\input\{([^}]+)\}', _body))
for src in (sorted(glob.glob('technosignatures_*.tex'))
            + sorted(glob.glob('tab_*.tex')) + sorted(glob.glob('tables/*.tex'))):
    live = (src.startswith('technosignatures_')
            or os.path.basename(src)[:-4] in _inputs)
    body = open(src, errors='ignore').read()
    for nm in sorted(set(STARPAT.findall(body))):
        raw = nm
        nm = nm.replace('~', ' ').replace('$', '').replace('−', '-').strip()
        nm = nm.rstrip('.,;:')
        canon = star_alias.designation(nm, tex=False)
        if canon not in CAT_DESIG:
            continue                     # not one of ours: a citation, a code
        if canon != nm:
            (FAIL if live else WARN).append(
                ('star in %s not in SIMBAD form' % os.path.basename(src),
                 raw, canon))
