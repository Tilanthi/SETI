...
EXP = json.load(open('corrected_export_v399.json'))['rows']
...
# v4.01 (R2-M2): the cost of the mask at other half-widths, quoted where the
# text argues the classification does not turn on the adopted tolerance.
# These were \MaskBWTwenty and \MaskBWHundred in the ungenerated frozen
# round-5 file, from the v3.32 extraction: 2.1 and 10 per cent, against the
# 2.3 and 8.7 the released catalogue gives.  Computed here with the same tube
# machinery as the adopted half-width, so they are one calculation at four
# settings, and the adopted setting must reproduce \MaskGrossPct exactly.
_ISL = [tuple(sorted((float(r['flo_GHz']), float(r['fhi_GHz'])))) for r in ROWS]
_BW = {}
for _tag, _half in (('Twenty', 20.0), ('Thirty', 30.0),
                    ('Fifty', 50.0), ('Hundred', 100.0)):
    _tb = merge([(x['f'] * (1 - _half / C_KMS), x['f'] * (1 + _half / C_KMS))
                 for x in DBF])
    _BW[_half] = 100.0 * sum(cost([iv], _tb) for iv in _ISL) / GROSS
    M('MaskBW' + _tag, '%.1f' % _BW[_half])
assert abs(_BW[HALF] - 100.0 * gross_lost / GROSS) < 1e-6, \
    'the half-width sweep disagrees with MaskGrossPct at the adopted width'

# R2-S1: the per-class split, on the SAME adopted tube set as MaskUnionPct
# so the three percentages are reconcilable. The union of a class is the
# merged extent of its own windows, so the parts do not sum to the total:
# Class A and Class B overlap in frequency.
_PCT = {}
for _tag, _cls in (('A', 'fine'), ('B', 'coarse')):
    _isl = merge([tuple(sorted((float(r['flo_GHz']), float(r['fhi_GHz']))))
                  for r in ROWS if r['resolution_class'] == _cls])
    _u = sum(y - x for x, y in _isl)
    _l = cost(_isl, TUBES_DB)
    M('MaskUnion' + _tag, '%.1f' % _u)
    M('MaskLost' + _tag, '%.2f' % _l)
    M('MaskPct' + _tag, '%.1f' % (100.0 * _l / _u if _u else 0.0))
    _PCT[_tag] = 100.0 * _l / _u if _u else 0.0
M('MaskPctRatioAB', '%.1f' % (_PCT['A'] / _PCT['B'] if _PCT.get('B') else 0.0))
...
# v3.80: the integration count comes from a peak-frequency freeze that
# does not cover blocks searched since, so some Class A windows have none.
# Transferability of the injection grid can only be judged for windows
# whose configuration is known; the rest are counted and reported, never
# assumed to be inside the grid.
A_ALL = [r for r in ROWS if r['search_class'] == 'A']
A = [r for r in A_ALL if r['n_int'] not in ('', None)]
M('GridNoNint', '%d' % (len(A_ALL) - len(A)))
...
M('GridKnownN', '%d' % len(A))
M('GridInsidePctAll', '%.0f' % (100.0 * len(inside) / len(A_ALL)))

# Per-band window counts for the summary table.  These were previously
# \BandWinThree..\BandWinEight from round8_calc.py, which reads the v3.31
# freeze and therefore knew only Bands 3-8: the table row summed to 431, not
# 443 (referee 1, point 4).  One macro, from the released catalogue.
_bw = collections.Counter(int(r['band']) for r in ROWS)
M('BandWinLabels', '/'.join(str(b) for b in sorted(_bw)))
M('BandWinCounts', '/'.join(str(_bw[b]) for b in sorted(_bw)))
assert sum(_bw.values()) == NWIN

# =====================================================================
# (6) Drift-resolved coverage counted in systems
# =====================================================================
# v3.80: use the FULL Class A population here. Earlier in this file `A`
# was narrowed to the windows whose integration count is known, for the
# injection-grid transferability test; drift-resolved COVERAGE is a
# different question and must not inherit that cut. Reusing the name cost
# five systems (60 where the catalogue has 65) and the audit caught it.
sysA = {r['system_id'] for r in A_ALL}
sysAll = {r['system_id'] for r in ROWS}
starA = {r['star_name'] for r in A_ALL}
...
# =====================================================================
# v3.85: read the re-queried census over all searched blocks, not the
# 104-block v3.51 freeze. The old assertion `len(POL) == both` passed only
# because the freeze contained no single-polarisation block; two of the 200
# now resolved do, so the assertion has been inverted into a report.
POL = json.load(open('polstates_v385.json' if os.path.exists('polstates_v385.json')
                     else 'polstates_v351.json'))
_hands = lambda v: all('XX' in str(x) and 'YY' in str(x)
                       for x in (v if isinstance(v, list) else [v]))
both = sum(1 for v in POL.values() if _hands(v))
cross = sum(1 for v in POL.values()
            if any('XY' in str(x) or 'YX' in str(x)
                   for x in (v if isinstance(v, list) else [v])))
M('PolCross', '%d' % cross)
