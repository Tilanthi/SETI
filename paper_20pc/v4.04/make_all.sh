...
python3 occurrence_v399.py          > /dev/null   # -> survey_numbers_round48.tex (R2-2: CWTFM / Transmitter Rate / hosting fraction); cross-asserts NSysClassA, UnionClassA, SysUnionAMed
python3 rfi_v399.py                 > /dev/null   # -> survey_numbers_round49.tex (R2-3: RFI vetting of the unattributed events)
...
python3 blockfate_v400.py           > /dev/null   # -> survey_numbers_round69.tex (R1-8: the fate of every in-scope block the original work list missed -- searched, no archive calibration, legacy calibration failure, disk-infeasible, or reachable and not attempted)
...
python3 projcodes_v400.py           > /dev/null   # -> survey_numbers_round67.tex, tab_projcodes_v400.tex (R2-num 28: the ALMA project codes, listed in the paper as ALMA policy requires; resolved EB -> member OUS -> proposal_id, and NAMES any block it cannot resolve).  # MUST follow make_numbers (reads NEB)
...
python3 make_fig_chain_v403.py      > /dev/null   # -> figures/chain_v403.pdf (Fig. 3: the A3 chain, with the visibility fit OUT of the column and reported alongside); MUST follow ledger_v403.py AND epoch_v403.py, whose E[max of 12] it reads rather than hard-coding
...
python3 ledgers_v404.py             > /dev/null   # -> survey_numbers_round87.tex (R2-7: closes the extension ledger (136 = 115 + 8 + 13, the 13 processed after the export snapshot), separates the two uncalibrated-block counts by their denominators, and quotes recurrence coverage with its criterion attached instead of the bare 79 per cent). MUST follow blockfate_v400.py, extension_v399.py and v363_calc.py, whose macros it reads back
python3 maskframe_v404.py           > /dev/null   # -> survey_numbers_round88.tex (v4.04: PROVES the line mask is evaluated topocentrically by re-deriving every released offset from the observed sky frequency against the 15-transition list the search ran; the paper said stellar frame and that was a documentation error). MUST follow v342_calc.py, whose catalogue and CAT_OLD literal it reads
