#!/usr/bin/env python3
# v3.46 page-trim variant: Figure 1 panels side by side, fonts rescaled for 1:1
...
import argparse
from star_alias import canon as _canon  # v3.85: alias merge
import collections
...
DEFAULT_DATA = os.path.join(HERE, "corrected_export_v399.json")  # v3.46: local copy;
...
def save(fig, path):
    # v3.46: tight bbox for the side-by-side Fig. 1 so no label is clipped.
...
# Referee: the plotted quantity for this survey must be the
# completeness-calibrated transmitter threshold, not the internal 5 sigma
# trigger. P90 is the effective (response- and smearing-corrected) power
# scaled by the injection-measured ratio StratPNinety/5, and the catalogue
# carries the effective power per window, so the conversion is exact.
def _p90_factor():
    import glob as _g, re as _re
    for _f in sorted(_g.glob(os.path.join(HERE, "survey_numbers*.tex"))):
        _m = _re.search(r"\\newcommand\{\\StratPNinety\}\{([0-9.]+)\}",
                        open(_f).read())
        if _m:
            return float(_m.group(1)) / 5.0
    return 6.1 / 5.0
...
def _eff_index():
    """Per-window powers, keyed (eb, lower edge), from the released
    catalogue: the frozen export carries only the trigger."""
    import csv as _csv
    idx, sel = {}, {}
    for fn in ("per_target_results_v3.99.csv",):
        path = os.path.join(HERE, fn)
        if not os.path.exists(path):
            continue
        for r in _csv.DictReader(open(path)):
            try:
                k = (r["eb"], round(min(float(r["flo_GHz"]),
                                        float(r["fhi_GHz"])), 4))
                idx[k] = float(r["eirp_eff_total_W"])
                if r.get("eirp_p90_sel_W"):
                    sel[k] = float(r["eirp_p90_sel_W"])
...
def p90_of(r):
    """Per-window 90 per cent recovery power through the complete selection.

    v4.01 (R2-M9.3): this plotted the TRIGGER-level P_90 and the axis said
    so, with the label "NOT a like-for-like sensitivity axis". The referee's
    point is that P_90^sel is the quantity the paper defines as the survey
    sensitivity, so it is the one the figure must show; the caveat that the
    comparison programmes quote their own 5-sigma thresholds belongs in the
    caption, where it now is. The catalogue measures P_90^sel per window.
    """
    ...
    eff = EFF.get(k)
    if eff is None:
        eff = r["eirp"] * 2.29          # survey median response correction
    return eff * P90FAC


def deepest_per_star(good):
    # v3.85: key on the canonical designation. Keying on the raw export
    # string put four stars in this figure twice, so Fig. 1 plotted 94
    # points against a caption that said 90.
    best = {}
    for r in good:
        s = _canon(r["star_name"])
        if s not in best or p90_of(r) < p90_of(best[s]):
            best[s] = r
    return best
...
def fig_eirp_context(good, outdir, report):
    ...
    e = np.array([p90_of(r) for r in best.values()])
    ...
    report["eirp_context"] = {
        ...
        "deepest_star": min(best, key=lambda k: p90_of(best[k])),
    ...
    xlo, xhi = 0.75, 2.6e4
    ylo, yhi = 7e11, 2e20   # headroom for the grouped legend, which would
                            # otherwise sit on the flagged-window stars
    ...
    # Moved out from under the grouped legend (v3.85).
    ax.text(1400, D2_NORM * 1400 ** 2 * 1.5, r"EIRP $\propto d^{2}$", fontsize=7.66,
    ...
    ax.scatter([r["dist_pc"] for r in sub], [p90_of(r) for r in sub],
               s=30, marker="*", color="k", zorder=5, label="flagged window")

    # --- literature thresholds -----------------------------------------------------
    # Referee: these must READ as a different kind of object, not as three
    # more points in the same cloud. Bigger, heavy black edge, and a halo,
    # so that "ours" and "other programmes" separate at a glance.
    for key in ("enriquez2017", "margot2023", "mason2024"):
        L = LIT[key]
        xs_, ys_ = [p[0] for p in L["pts"]], [p[1] for p in L["pts"]]
        ax.scatter(xs_, ys_, s=L["size"] * 2.6, marker=L["marker"],
                   color="white", edgecolor="none", zorder=5)
        ax.scatter(xs_, ys_, s=L["size"] * 1.9, marker=L["marker"],
                   color=L["colour"], edgecolor="k", linewidths=0.9,
                   zorder=6, label=L["label"])
    L = LIT["price2020"]
    ...
    ax.set_xlim(xlo, xhi)
    ax.set_ylim(ylo, yhi)
    # R1-s2: the panel title, not the caption, has to carry this -- figures
    # travel without their captions.
    ax.set_title("(a) parameter-space location; vertical sensitivities "
                 "not directly comparable", fontsize=7.4, color="0.2",
                 pad=3.0)
    ax.set_xlabel("distance (pc)", labelpad=1.0)
    # The ordinate must carry the caveat itself: a reader who takes this
    # panel as a like-for-like sensitivity comparison with a few-Hz
    # narrowband backend has drawn the wrong conclusion, and a caption is
    # too easy to skip.
    # Referee: the axis itself must say that the comparison is not
    # like for like, because a reader who looks only at figures will
    # never reach the caption.
    # R2-M9.3: the axis is the headline sensitivity, P_90^sel. The
    # comparison programmes quote their own 5-sigma thresholds, which is a
    # more permissive convention; that is said in the caption rather than
    # on the axis.
    ax.set_ylabel("$P_{90}^{\\rm sel}$, this survey (W)\n"
                  "literature points are each programme's own "
                  "5$\\sigma$ threshold", labelpad=1.5)
    ...
    # Referee: the legend must say which entries are this survey and which
    # are other programmes. Build it from proxies at a controlled size --
    # the literature markers are deliberately oversized in the panel and
    # would dominate the key if taken from the artists.
    blank = Line2D([], [], ls="none", marker="none")
    handles = [
        blank,
        Line2D([], [], ls="none", marker="v", mfc="none", mec="#56B4E9",
               mew=0.7, ms=3.6),
        Line2D([], [], ls="none", marker="o", mfc="#0072B2", mec="none", ms=3.6),
        Line2D([], [], ls="none", marker="*", color="k", ms=6.0),
        blank,
    ]
    labels = [
        r"$\bf{This\ survey}$",
        "deepest window per star, Class B (%d)" % int((~fine_star).sum()),
        "deepest window per star, Class A (%d)" % int(fine_star.sum()),
        "flagged window",
        r"$\bf{Other\ programmes}$",
    ]
    for key in ("enriquez2017", "margot2023", "mason2024"):
        L = LIT[key]
        handles.append(Line2D([], [], ls="none", marker=L["marker"],
                              mfc=L["colour"], mec="k", mew=0.8, ms=5.2))
        labels.append(L["label"])
    handles.append(Line2D([], [], color=LIT["price2020"]["colour"], lw=1.4))
    labels.append(LIT["price2020"]["label"])
    leg = ax.legend(handles, labels, loc="upper left", fontsize=6.5,
                    handletextpad=0.5, borderpad=0.35, labelspacing=0.42,
    ...
    axb.text(6.5, 3.2e2, "Mason+25  30.5 kHz", fontsize=6.86,
    ...
    axb.set_title("(b) native spectral resolution, directly comparable",
                  fontsize=7.4, color="0.2", pad=3.0)
    ...
    # R2-m: the referee asked for the warning to be legible ON the figure,
    # not only in the caption, because the panel invites a direct
    # comparison of depths that is not valid across these resolutions.
    axb.text(0.025, 0.035,
             "Same physical quantity, different\nconventions: channel widths "
             "differ by\n%.0f decades (see caption)." % span_dex,
             transform=axb.transAxes, fontsize=6.4, color="0.15",
             ha="left", va="bottom", linespacing=1.3,
             bbox=dict(boxstyle="round,pad=0.32", facecolor="1.0",
                       edgecolor="0.35", linewidth=0.6, alpha=0.93))
    ...
    save(fig, os.path.join(outdir, "eirp_context.pdf"))
    # R1-9 (v3.99): the panels are also written separately, so the main
    # text can carry the channel-width comparison -- the distinctive
    # point -- while the cross-survey EIRP panel, which needs three
    # warnings to be read correctly, goes to an appendix.
    import matplotlib.pyplot as _plt
    for _which, _name in ((axR if False else ax, "eirp_context_a"),
                          (axb, "eirp_context_b")):
        _f = _plt.figure(figsize=(3.55, 2.75))
        _new = _f.add_axes([0.20, 0.17, 0.77, 0.74])
        for _l in _which.get_lines():
            _new.plot(_l.get_xdata(), _l.get_ydata(), color=_l.get_color(),
                      lw=_l.get_linewidth(), ls=_l.get_linestyle(),
                      marker=_l.get_marker(), ms=_l.get_markersize(),
                      mfc=_l.get_markerfacecolor(),
                      mec=_l.get_markeredgecolor(), zorder=_l.get_zorder())
        for _c in _which.collections:
            try:
                _o = _c.get_offsets()
                _new.scatter(_o[:, 0], _o[:, 1], s=_c.get_sizes(),
                             facecolors=_c.get_facecolors(),
                             edgecolors=_c.get_edgecolors(), zorder=3)
            except Exception:
                pass
        for _t in _which.texts:
            _new.text(_t.get_position()[0], _t.get_position()[1],
                      _t.get_text(), fontsize=_t.get_fontsize(),
                      color=_t.get_color(), ha=_t.get_ha(), va=_t.get_va(),
                      rotation=_t.get_rotation())
        _new.set_xscale(_which.get_xscale()); _new.set_yscale(_which.get_yscale())
        _new.set_xlim(_which.get_xlim()); _new.set_ylim(_which.get_ylim())
        _new.set_xlabel(_which.get_xlabel(), fontsize=8)
        _new.set_ylabel(_which.get_ylabel(), fontsize=7)
        _new.grid(True, which="major", alpha=0.5)
        _f.savefig(os.path.join(outdir, _name + ".pdf"),
                   bbox_inches="tight", pad_inches=0.02)
        _plt.close(_f)
    return os.path.join(outdir, "eirp_context.pdf")
...
def main():
    ...
    p2 = p1  # completeness_surface is not used in v3.46
