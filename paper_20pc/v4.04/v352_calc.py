...
Inputs (all frozen; all in this directory unless noted):
  corrected_export_v399.json        the per-window search export (512 controls)
  per_target_results_v3.99.csv    the released catalogue (431 windows)
  archive_meta_v381.json          per-EB array, frequency support
  ...
  technosignatures_40pc_v3.81.tex parsed, read-only, for the transfer bracket
...
import calendar
import collections
from star_alias import canon as _canon, sysname as _sysname  # v3.85 alias merge
import csv
import json
import math
import os
...
def load_windows():
    ...
    d = json.load(open(os.path.join(HERE, 'corrected_export_v399.json')))
    rows = d['rows']
...
# PAIRS now lives in star_alias.py (v3.85): six copies existed.

    # v3.85: alias-merge before the pair collapse; this local copy of the
    # map did not know about the four duplicate name strings.
    for r in good:
        r['sys'] = _sysname(r['star_name'])
...
# The two Band 9/10 blocks restored in v3.60 are outside the archive-metadata
# snapshot. Their recorded primary-beam FWHM, 7.3 arcsec at 856 GHz, is the
# 12 m value (the 7 m dish would give 12.6 arcsec), so they are taken as 12 m.
ARRAY = {eb: (EBS.get(eb, {}).get('array') or '12m') for eb in {r['eb'] for r in G}}
...
RATIO = [eff_res_ratio(F(r['flo_GHz']), F(r['fhi_GHz']), F(r['chanw_Hz']), r['eb'])
         for r in ROWS]
# The Band 9/10 blocks restored in v3.60 are outside the archive-metadata
# snapshot, so their ratio is unknown and they take ALMA's default, which is
# the modal value and the larger response correction. Everything else is
# measured, and the count of unknowns is published rather than hidden.
_unknown = [i for i, x in enumerate(RATIO) if x is None]
assert all(int(ROWS[i]['band']) in (9, 10) for i in _unknown), \
    'unknown frequency support outside Bands 9-10'
M('NNoArchRes', '%d' % len(_unknown))
_known = [x for x in RATIO if x is not None]
_default = collections.Counter(round(x, 3) for x in _known).most_common(1)[0][0]
RATIO = [(_default if x is None else x) for x in RATIO]
...
# v3.60: \NHanTwo and \NHanOther are OWNED by v343_calc.py, which counts only
# the windows the archive metadata actually covers (403 of 431).  This script
# needs a ratio for every row of the kernel model, so it substitutes the
# default for the twelve Band 9/10 rows -- but that substitution is an
# assumption, not a measurement, and it must not be allowed to leak into the
# appendix sentence that says "exactly 2.00 in \NHanTwo of the \NHanKnown
# windows".  So assert the relation instead of re-emitting the macros.
_owned_two = int(texval('survey_numbers_round13.tex', 'NHanTwo'))
_owned_other = int(texval('survey_numbers_round13.tex', 'NHanOther'))
assert _n_two == _owned_two + len(_unknown), \
    'assumed-default rows do not account for the difference: %d vs %d + %d' % (
        _n_two, _owned_two, len(_unknown))
assert len(RATIO) - _n_two == _owned_other, 'other-ratio count drifted'
...
# ---- v4.01 (R2-M2/M3b).  The two routines above are the SUPERSEDED top-hat
# channel model, kept because the archive-ratio argument below was originally
# made with them and the reader should be able to see what changed.  The
# correction itself now comes from cresp_v401.py, which computes the delivered
# channel's response from the correlator's lag window; this file reads it
# rather than carrying a third implementation of the same physics.
_CRESP = json.load(open(os.path.join(HERE, 'cresp_v401.json')))
_rho_n = {int(k): np.array(v) for k, v in _CRESP['rho_by_n'].items()}
_fwhm_n = {int(k): v for k, v in _CRESP['fwhm_by_n'].items()}
_fac_one = 1.0 / float(np.median(_rho_n[1]))
# the catalogue must have published exactly the model this file reads; if the
# two ever disagree, one of them is reading a stale json
assert abs(_fac_one - F(texval('survey_numbers_round12.tex', 'HanFacMed'))) < 5e-3, _fac_one
# The superseded model could only BRACKET the averaged windows' reported
# resolution between N = 2 and N = 4. The validated response reproduces it
# outright at N = 2, and is inconsistent with N = 4, which is a stronger
# statement and the one now made.
N_AVG_HI, N_AVG_LO = 2, 4
assert abs(_fwhm_n[N_AVG_HI] - _other_ratio) / _other_ratio < 0.01, (
    'the response model predicts %.3f delivered channel spacings for the '
    'averaged windows and the archive reports %.3f'
    % (_fwhm_n[N_AVG_HI], _other_ratio))
assert abs(_fwhm_n[N_AVG_LO] - _other_ratio) / _other_ratio > 0.05, (
    'averaging by %d is no longer distinguishable from averaging by %d'
    % (N_AVG_LO, N_AVG_HI))
M('HanFacAvgMedFour', '%.2f' % (1.0 / float(np.median(_rho_n[N_AVG_LO]))))
M('HanFacAvgMedTwo', '%.2f' % (1.0 / float(np.median(_rho_n[N_AVG_HI]))))
...
M('NStarTwelveM', '%d' % len({_canon(r['star_name']) for r in w12}))
...
# v4.01 (R2-M5.4).  The text quotes P(>= SbrN) and used \ClustPfive, which is
# P(>= 5).  The observed count is not 5, so the paper printed 0.23 for a tail
# it labelled P(>= 14).  Key the macro to the OBSERVED count and let the
# generator read it, so the label and the number cannot part company again.
# Clustering does not rescue the number: for this mean the block-clustered
# convolution and a plain Poisson agree to three significant figures, which is
# asserted rather than asserted-in-prose.
import math as _mth
_nobs = int(F(texval('survey_numbers.tex', 'SbrN')))
_mu = float((_k * _pmf).sum())
M('ClustNObs', '%d' % _nobs)
M('ClustPobs', sci(float(_pmf[_nobs:].sum())))
_t, _pois = _mth.exp(-_mu), 0.0
for _i in range(0, 400):
    if _i >= _nobs:
        _pois += _t
    _t *= _mu / (_i + 1)
M('ClustPobsPois', sci(_pois))
assert abs(float(_pmf[_nobs:].sum()) - _pois) < 0.15 * max(_pois, 1e-12), (
    'the block-clustered tail (%.3g) and the Poisson tail (%.3g) disagree; '
    'the claim that clustering does not change the answer is no longer true'
    % (float(_pmf[_nobs:].sum()), _pois))
...
# v4.01 (R2-M5.2): the paper must quote P(>= N_observed), not P(>= 1).  With
# two unattributed events observed, P(>=1) = 0.59 answers a question nobody
# asked.  The observed count is read from the catalogue's own disposition
# column so the tail cannot be keyed to a stale number, which is exactly how
# \ClustPfive came to be typeset against \SbrN = 14.
import csv as _csv
_nun = sum(1 for _r in _csv.DictReader(open('per_target_results_v3.99.csv'))
           if _r['disposition'].strip().lower().startswith('unattributed'))
assert 0 < _nun < len(_pmf1), 'unattributed count %d out of range' % _nun
M('ClustStageNObs', '%d' % _nun)
M('ClustStagePobs', '%.2f' % float(_pmf1[_nun:].sum()))
...
# The transfer is now measured rather than assumed, so the endpoints come from
# the stratified campaign's own macros instead of from a sentence in the text.
# v3.85 (Q20): this multiplied a recovery PERCENTAGE by a POWER-transfer
# factor and clamped the product at 100, which is dimensionally invalid --
# the transfer bracket scales the power at which a given recovery is
# reached, not the recovery reached at a fixed power. The bracket belongs
# on the threshold, where the paper already applies it, and these two
# macros are withdrawn rather than corrected.
BRA_LO = F(texval('survey_numbers_round23.tex', 'StratTransferLo'))
BRA_HI = F(texval('survey_numbers_round23.tex', 'StratTransferHi'))
assert BRA_LO and BRA_HI
...
SMEAR_FLOOR = 0.99
# v3.80: windows with no smearing figure are excluded here as in every
# other census of the column; an empty cell is not a zero.
M('NSmearLo', '%d' % sum(1 for r in ROWS
                         if r['eta_smear'] != ''
                         and F(r['eta_smear']) < SMEAR_FLOOR))
...
CATALOGUE_FILE = 'per_target_results_v3.99.csv'
MAIN_TEX = [f for f in os.listdir(HERE)
            if f.startswith('technosignatures_40pc_v') and f.endswith('.tex')][0]
