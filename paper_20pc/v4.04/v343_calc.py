...
`per_target_results_v3.99.csv` (the released catalogue), the frozen export
...
CSV = HERE + '/per_target_results_v3.99.csv'
...
# v3.80: on the completed sweep some windows carry no smearing figure at
# all (the column is empty when the drift track cannot be evaluated), and
# the helper raises rather than returning a default -- correctly, since a
# missing value must not silently become 1.0.  Census the windows that have
# one, and report how many do not.
_HASSM = [r for r in ROWS if r.get('eta_smear', '') != '']
M('NSmearNoValue', '%d' % (len(ROWS) - len(_HASSM)))
SM = sorted((r for r in _HASSM if f(r, 'eta_smear') < 0.99),
...
M('NSmearOk', '%d' % (len(_HASSM) - len(SM)))
...
# v3.80: beta Pic now carries five flagged Band 3 windows and three Band 6,
# so this loop emitted \BpicBThreeTsym five times and LaTeX refused the
# repeats ("Command already defined", 16 errors in the build). The text
# means the DEEPEST window of each band, which is what it always was when
# there was only one; make that explicit.
_bybandBP = {}
for r in BP:
    b = r['band']
    if b not in _bybandBP or f(r, 'star_snr') > f(_bybandBP[b], 'star_snr'):
        _bybandBP[b] = r
M('BpicNFlagThree', '%d' % sum(1 for r in BP if r['band'] == '3'))
M('BpicNFlagSix', '%d' % sum(1 for r in BP if r['band'] == '6'))
for b, r in sorted(_bybandBP.items()):
    tag = {'3': 'Three', '6': 'Six'}[b]
    T = f(r, 'star_snr')
...
# R2-S10: the title states the survey's frequency span, so generate it
_lo = min(min(f(r, 'flo_GHz'), f(r, 'fhi_GHz')) for r in ROWS)
_hi = max(max(f(r, 'flo_GHz'), f(r, 'fhi_GHz')) for r in ROWS)
M('SurvFreqLo', '%d' % round(_lo))
M('SurvFreqHi', '%d' % round(_hi))
# R2-m1 (v4.04): the title must describe the PRIMARY experiment the abstract
# reports, which is the Class A fine-channel search, not the union of both
# classes.  Generate its own span rather than retyping the referee's figures.
_A = [r for r in ROWS if r['resolution_class'].startswith('fine')]
assert _A, 'no Class A rows'
_alo = min(min(f(r, 'flo_GHz'), f(r, 'fhi_GHz')) for r in _A)
_ahi = max(max(f(r, 'flo_GHz'), f(r, 'fhi_GHz')) for r in _A)
# v4.04 (intsweep): "The 112 Band 3 windows searched here" was a prose
# literal that happens to be a catalogue quantity.  Generate it.
M('NWinBandThree', '%d' % sum(1 for r in ROWS if r['band'] == '3'))
M('SurvFreqLoA', '%d' % round(_alo))
M('SurvFreqHiA', '%d' % round(_ahi))
# The Class A span must sit inside the survey span, or one of the two is
# computed over the wrong row set.
assert _lo <= _alo and _ahi <= _hi, (_lo, _alo, _ahi, _hi)
# ... and it must be strictly narrower at the low end, which is the whole
# reason the title changes: the Band 3 coarse windows reach lower than any
# fine one.
assert _alo > _lo, (_alo, _lo)
M('UnionBandThree', '%.1f' % B3)
M('UnionBandsFourEight', '%.1f' % HI)
# The label for that coverage was hand-written as "Bands 4--8" and the
# survey reaches Band 10 (16 Band 9/10 windows in the catalogue), so the
# prose named a narrower range than the number it was attached to.
# Derive the label from the same rows that set the number.
_hib = sorted({int(r['band']) for r in ROWS if r['band'] != '3'})
M('UnionHiBandLabel', 'Bands~%d--%d' % (_hib[0], _hib[-1]))
...
# v3.80: the per-window hit counts come from pipeline_peakfreq_v342.json,
# a freeze built when the catalogue held 443 windows. Crossings in blocks
# searched since then have no entry, so count over the windows that have
# one and say how many that is, rather than reading through a None -- the
# same discipline as the smearing census above.
CRS_ALL = [r for r in ROWS if r['crossing'] == 'True']
# The window COUNTS are a property of the catalogue and must stay over the
# whole crossing population -- v344_calc.py's Poisson comparison reads
# \NHitWindowsRest and pairs it with an expectation computed over the same
# population, so silently narrowing it here would make the two disagree
# (caught in v3.80 by v351_calc.py's cross-check, which is what that
# cross-check is for). Only the stored HIT SUMS are restricted, to the
# windows the v3.42 peak-frequency freeze covers.
CRS = [r for r in CRS_ALL if r.get('_pk')]
M('NHitWindowsNoPk', '%d' % (len(CRS_ALL) - len(CRS)))
...
M('NHitWindows', '%d' % len(CRS_ALL))
M('NHitWindowsRest', '%d' % (len(CRS_ALL)
                             - sum(1 for r in CRS_ALL
...
# v3.80: a window can be recorded as carrying a crossing without a
# released crossing FREQUENCY (the column is written only where the
# pipeline stored the peak). Edge proximity is a statement about where in
# the band the crossing sat, so it can only be made for those.
CROSS = [r for r in ROWS
         if r['crossing'] == 'True' and r.get('f_cross_GHz', '') != '']
M('NCrossWin', '%d' % len(CROSS))
M('NCrossNoFreq',
  '%d' % (sum(1 for r in ROWS if r['crossing'] == 'True') - len(CROSS)))
EDGE = []
for r in CROSS:
    i, nch = chan_index(r)
    frac = min(i, nch - i) / nch
    EDGE.append((frac, r, i, nch))
# sort on the fraction alone: the tuples carry a dict, so a bare sort()
# falls through to comparing dicts whenever two windows tie, which they now
# do because the repaired extraction retains a peak frequency for far more
# windows than the original did
...
# release folder: `archive_meta_v381.json` (ALMA obscore + datalink harvest)
...
# v3.80: count over the CATALOGUE's blocks, not over every block in the
# metadata file. The re-harvest covers 482 blocks while the catalogue holds
# 479, and counting the file gave 323 where the catalogue gives 322 -- a
# disagreement v363_calc.py's cross-assertion caught immediately.
_CAT_EBS = {r['eb'] for r in ROWS}
n7 = sum(1 for e in _CAT_EBS if EBS.get(e, {}).get('array') == '7m')
M('NEbSevenM', '%d' % n7)
...
# ---- v4.01 (R2-M2): the drift-grid discretisation loss --------------------
# Was \DriftGridFineHalf in the ungenerated frozen round-7 file, carried from
# the v3.31 export.  The grid is uniform in nu-dot over n_drift trials
# spanning +-drift_max, so the step is 2 drift_max/(n-1) and a carrier midway
# between two trials retains half of it.  Over the window's own track that
# residual rate sweeps step/2 x T of frequency; in native channels that is
# the lever arm the text quotes.  The number reported is the worst case over
# the class, so it is a bound and not a typical window.
def _gridhalf(cls):
    worst = 0.0
    for r in ROWS:
        if r['search_class'] != cls:
            continue
        n = f(r, 'n_drift_trials')
        if n < 2:
            continue
        step = 2.0 * f(r, 'drift_max_Hz_s') / (n - 1.0)
        worst = max(worst, 0.5 * step * f(r, 'on_source_s') / f(r, 'chanw_Hz'))
    return worst


M('DriftGridFineHalf', '%.2f' % _gridhalf('A'))
M('DriftGridCoarseHalf', '%.2f' % _gridhalf('B'))
