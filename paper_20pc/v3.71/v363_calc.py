...
M('PbFloorResp', '0.5')            # response floor adopted in v3.71
...
# interval is an order of magnitude wider.  `tailboot_v371.py` recomputes the
...
_TB = json.load(open('tailboot_v371.json'))
...
# stated in the text.
_CM = json.load(open('campaign_v371.json'))
...
for _i, _e in enumerate(_u[:len(_ORD)]):
    ...
    M('CampUn%sRepT' % _k, '%.2f' % _e['repeat_best_t'])
    M('CampUn%sRepNge' % _k, '%d' % _e['repeat_nge'])
...
# have now been searched.  `searched_ebs_v371.txt` is the list of execution
# blocks carrying a search product on the host, harvested read-only.
_all = {l.strip() for l in open('searched_ebs_v371.txt') if l.strip()}
