...
shipped here:
  ...
  Fig. 5  <- localnull_code/ctrlmax.json   (the window's 512 control statistics)
            per_target_results_v3.71.csv   (T_star and the ring maximum)
  Fig. 7  <- per_target_results_v3.71.csv  (window centre, nominal EIRP, class)
...
def fig_sensitivity_2d():
    ...
    # v3.71 (round 2, referee D1): P_eff,total is a PER-WINDOW catalogue column
    ...
    open(os.path.join(HERE, "peff_range_v371.tex"), "w").write(
