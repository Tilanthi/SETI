...
Exit status is non-zero if any check fails.  Writes `audit_v371.json`.
"""
...
MAC = {}
for fn in sorted(glob.glob('survey_numbers*.tex')) + ['peff_range_v371.tex',
                                                      'regen_count.tex']:
...
R = list(csv.DictReader(open('per_target_results_v3.71.csv')))
...
CM = json.load(open('campaign_v371.json'))
...
ALL = {l.strip() for l in open('searched_ebs_v371.txt') if l.strip()}
...
# tail factor
TB = json.load(open('tailboot_v371.json'))
...
try:
    ...
    figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}',
                                 open('technosignatures_40pc_v3.71.tex').read())))
    figreport = {}
    for g in figs:
    ...
    json.dump(figreport, open('audit_figures_v371.json', 'w'), indent=1)
except ImportError:
...
json.dump({'pass': len(OK), 'fail': [list(map(str, x)) for x in FAIL]},
          open('audit_v371.json', 'w'), indent=1)
