...
# M-dwarf row out of tab_selection.tex (v3.71, referee 2 M1).
...
python3 tailboot_v371.py            > /dev/null   # -> tailboot_v371.json (clustered tail CI)
...
python3 audit_numbers_v371.py       > /dev/null   # forensic number audit; fails the build if any number is stale
