...
assert len(GOOD) == 443, len(GOOD)   # 431 released + 12 Band 9/10 restored

# ---------------------------------------------------------------- unit repair
# v3.66 (referee 1, point 5).  The twelve Band 9/10 rows restored in v3.60 were
...
ARCH = json.load(open('archive_meta_v343.json'))
EBS = ARCH['ebs']
...
HAN_RATIO_TOL = 0.05
N_AVG = 2
_ua = np.linspace(-0.5 * N_AVG, 0.5 * N_AVG, 2001)
_fra = np.array([averaged_peak_fraction(u, N_AVG) for u in _ua])
HAN_AVG_BEST, HAN_AVG_WORST = _fra.max(), _fra.min()
HAN_AVG_MED = float(np.median(_fra))
# the averaged windows' median IS their best case, so taking it is the
# conservative reading of the correction for those windows
assert abs(HAN_AVG_MED - HAN_AVG_BEST) < 1e-12, (HAN_AVG_MED, HAN_AVG_BEST)
...
for r in GOOD:
    ...
    # an unknown ratio would fall back on ALMA's default, the larger
    ...
    hb, hw, hm = ((HAN_AVG_BEST, HAN_AVG_WORST, HAN_AVG_MED) if r['han_avg']
                  else (HAN_BEST, HAN_WORST, HAN_MED))
...
assert all(r['band_x'] in (9, 10) for r in _noratio), \
    'window with no archive resolution outside Bands 9-10'
...
assert (_neff_arec, _neff_twice) == (0, 1), (_neff_arec, _neff_twice)
assert '%.1e' % min(_deep_eff.values()) == '3.7e+13', min(_deep_eff.values())
...
def released_name(n):
    """The designation as released: signs restored, whitespace collapsed."""
    return ' '.join(NAME_REPAIR.get(n, n).split())
...
COLS = ['star_name', 'system_id', 'band', 'eb', 'dist_pc', 'flo_GHz', 'fhi_GHz',
        ...
        'disposition']
with open('per_target_results_v3.66.csv', 'w', newline='') as fh:
    w = csv.writer(fh)
    w.writerow(COLS)
...
print('  window-specific (E1): %d windows at ratio %.3f keep x%.2f; %d with '
      ...
      % (len(GOOD) - NHAN_AVG, HAN_RATIO, 1 / HAN_MED, NHAN_AVG, N_AVG,
...
print('catalogue -> per_target_results_v3.66.csv (%d rows, %d columns)'
      % (len(GOOD), len(COLS)))
