# Building technosignatures_20pc_v3.30

    python3 survey_stats.py      # -> survey_stats.json
    python3 make_numbers.py      # -> survey_numbers.tex  (ALL numbers in the paper)
    pdflatex technosignatures_20pc_v3.30.tex   # x3, converges on the third

## Single source of truth for every number
From v3.30 the manuscript contains no typed digits for survey quantities. It
...
0, and prints no warning; PDF text extraction then breaks. Verify:
    ...
    python3 -c "import pymupdf;d=pymupdf.open('technosignatures_20pc_v3.30.pdf');print({f[2] for p in range(d.page_count) for f in d[p].get_fonts()})"
