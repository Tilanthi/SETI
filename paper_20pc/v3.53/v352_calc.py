#!/usr/bin/env python3
"""v3.53 generator -> survey_numbers_round20.tex

Single-sources the quantitative blocks the v3.53 cycle adds, so that nothing
...
Inputs (all frozen; all in this directory unless noted):
  ...
  technosignatures_20pc_v3.53.tex parsed, read-only, for the transfer bracket
...
F = float
ROWS = list(csv.DictReader(open(os.path.join(HERE, 'per_target_results_v3.53.csv'))))
...
_applied = [F(r['theta_pb_arcsec']) / ARCSEC /
            ...
            for r in ROWS]
assert max(_applied) - min(_applied) < 1e-6, 'catalogue theta_pb is not one convention'
...
# HOOddCtrlNinetyNine, HOOddHits and HOOddDrift are NOT emitted: the frozen
# campaign record carries neither the control percentiles, nor the hit count,
# nor the fitted drift for this window.  See the build report.
...
_tex = open(os.path.join(HERE, 'technosignatures_20pc_v3.53.tex')).read()
