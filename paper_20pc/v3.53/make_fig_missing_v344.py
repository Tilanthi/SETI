...
shipped here:
  ...
  Fig. 5  <- localnull_code/ctrlmax.json   (the window's 512 control statistics)
            per_target_results_v3.53.csv   (T_star and the ring maximum)
  Fig. 7  <- per_target_results_v3.53.csv  (window centre, nominal EIRP, class)
...
def fig_sensitivity_2d():
    ...
    ax.set_ylabel(r"nominal 5$\sigma$ trigger EIRP (W)", fontsize=10)
    ax.tick_params(labelsize=9)
