...
def fig_eirp_context(good, outdir, report):
    ...
    survey_chan = "%s median channel" % _chan_label(cw_med)
    survey_chan_range = "%s to %s" % (_chan_label(cw_lo), _chan_label(cw_hi))
    ...
    # channel width of this survey, called out immediately above its own
    # points.  No leader: a leader to a 85-point cloud has no single target
    # and would cross the data.
    ax.text(2.9, 9.0e17, "this survey: %s\n(%s)" % (survey_chan,
                                                    survey_chan_range),
            fontsize=5.8, color="#0072B2", ha="left", va="center", zorder=7,
            linespacing=1.3,
            bbox=dict(facecolor="white", edgecolor="#0072B2",
                      linewidth=0.35, alpha=0.92, pad=1.6))

    # --- literature --------------------------------------------------------------------
    # channel-width annotations: (x, y) of the text, and the point it belongs to
    CHAN_XY = {
        "enriquez2017": (50.0, 3.9e12),     # below its diamond
        "margot2023": (6135.0, 1.9e17),     # above its far square
        "mason2024": (1010.0, 2.3e18),      # above its triangle
    }
    for key in ("enriquez2017", "margot2023", "mason2024"):
        ...
        ax.scatter(xs_, ys_, s=L["size"], marker=L["marker"], color=L["colour"],
                   ...
                   label=L["label"])
        tx, ty = CHAN_XY[key]
        ax.text(tx, ty, L["chan_label"], fontsize=5.6, color=L["colour"],
    ...
    for (_, y), nm in zip(L["pts"], L["tick_names"]):
        ...
        ax.text(xlo * 1.75, y, "%s, %s" % (nm, L["chan_label"]), fontsize=5.2,
                color=L["colour"], ha="left", va="center", zorder=6)
    ...
    fig.text(0.5, 0.008,
             ...
             "native channel width, which spans %.0f decades across the "
             "surveys plotted." % span_dex,
             ha="center", va="bottom", fontsize=5.2, color="0.35",
             linespacing=1.35)
    fig.get_layout_engine().set(rect=(0.0, 0.062, 1.0, 0.938))
