...
with open('tab_unattributed_v383.tex', 'w') as fh:
    ...
    for r in _unatt:
        ...
        if rep:
            ...
            _b = _baselines(r, rep)
            _sep = [abs(_mjd(q['eb']) - _mjd(r['eb'])) for q in rep
                    if _mjd(q['eb']) is not None and _mjd(r['eb']) is not None]
            _res = [d for d in _sep if d > 0.01]      # resolvable
            if _res:
                def _fmt(d):
                    return ('%.0f\\,d' % d if d < 365
                            else '%.1f\\,yr' % (d / 365.25))
                rp = '%d; $T_\\star=%.2f$; %s--%s' % (
                    len(rep), F(best['star_snr']), _fmt(min(_res)),
                    _fmt(max(_res)))
                if len(_res) < len(_sep):
                    rp += '$^{b}$'
            else:
                rp = '%d; $T_\\star=%.2f$; same day$^{b}$' % (
                    len(rep), F(best['star_snr']))
        else:
            rp = 'none'
