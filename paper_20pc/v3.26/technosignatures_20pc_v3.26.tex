...
the released per-target file (Appendix~\ref{app:table}) each entry carries its own row and distance, and
...
(\S\ref{sec:popanomaly}). Throughout, ``5$\sigma$'' denotes this local
...
molecular-line census are defined in \S\ref{sec:extended} and
\S\ref{sec:linecatalogue}; every headline limit, candidate
...
noise, so no threshold in the released per-target file (Appendix~\ref{app:table}) is affected by the
mask.
...
(EIRP$_{90} > 2\,$EIRP$_{5\sigma}$; \S\ref{sec:injectrecovery}). It
...
the released per-target file (Appendix~\ref{app:table}); it is stated once here and not repeated at each
occurrence.}
\smallskip
\noindent\rule{\columnwidth}{0.4pt}
\medskip
...
31.25\,MHz (median 15.625\,MHz; the released per-target file (Appendix~\ref{app:table})), and the trial
...
$\eta_{\rm smear} < 0.99$ (marked $^{\ast}$ in the released per-target file (Appendix~\ref{app:table});
...
the pipeline or tables). Velocities enter at one place only, the
...
catalogued molecular rest frequency; its residual error is the
catalogue radial-velocity uncertainty alone (a few km\,s$^{-1}$ at
20\,pc). The stacking behind the search statistic is
...
factor ${\sim}2$, and the measured channel-phase
...
the archival channelisation) -- with the smearing table and the
channel-width verification, is in Appendix~\ref{app:conventions}
...
Band~7: four of eight windows searched; the released per-target file (Appendix~\ref{app:table})), so the
...
marked in the released per-target file (Appendix~\ref{app:table}), with full multi-window coverage
deferred to the next release. Each searched window yields its own
EIRP$_{5\sigma}$ limit as a separate row of the released per-target file (Appendix~\ref{app:table});
...
\S\ref{sec:linecatalogue}) using a $\pm50$\,km\,s$^{-1}$
...
the released per-target file (Appendix~\ref{app:table}) and excluded from the headline ensemble
...
\S\ref{sec:results} throughout to two benchmarks for real human
...
mobile-network leakage (EIRP of order $4\times10^{9}$\,W): the most
powerful deliberate directed transmitter an Earth-equivalent
civilisation could point at us, and unintentional background leakage
with no signalling intent.
...
(Appendix~\ref{app:closure}). The machinery is implemented and
...
(specification and error model in Appendix~\ref{app:continuum}). The
...
(kernel and caveats in Appendix~\ref{app:freqoccupancy}); the null
...
Table~\ref{tab:supp} summarises every analysis with its validation
status and whether it enters the headline limits.
...
searched frequency range, are given in the released per-target file (Appendix~\ref{app:table})
(online-only Appendix~\ref{app:table}).

\subsection{Per-star summary}
\label{sec:starsummary}

The one-line-per-star view -- Table~\ref{tab:starsummary}
(online-only), complementing the per-window the released per-target file (Appendix~\ref{app:table}) --
gives the deepest nominal EIRP$_{5\sigma}$ threshold among each star's
windows, its band, and that band's continuum-lane result; `Bands'
lists the ALMA bands with searched windows. Two stars of the release
are continuum-only. All values are read directly from
the released per-target file (Appendix~\ref{app:table}) and add no new numbers; EIRP values are nominal
native-channel thresholds under the boxed rule of \S\ref{sec:method},
and continuum fluxes are primary-beam corrected.
...
the released per-target file (Appendix~\ref{app:table}) are, with forensic detail in the online-only
appendices:
...
the cross-target occupancy screen (\S\ref{sec:freqoccupancy}) covers
...
block-permutation model above remains the operative release-wide
estimate, with the symmetric statistic a validation subset measured
wherever the data allow. The AU~Mic candidate also fails the
...
Band~6 window without a candidate (the released per-target file (Appendix~\ref{app:table})); shown
...
The on-source integrations quoted here were reconstructed from
pipeline bookkeeping rather than retained natively, so we audited them
against the ALMA archive's own exposure accounting
(\texttt{ivoa.obscore} science-intent exposure per unit set; TAP query
of 2026 September~8; method, per-block numbers, and caveats in
online-only Appendix~\ref{app:audit}). The audit covers all 227
window-level records in 58 star--execution-block groups (the 59
...
dump-bookkeeping artefact, already corrected to the archive's 877\,s
in Table~\ref{tab:duty} -- and CD$-$38~10980 by 7\%).

Three statements bound the scientific impact. First, no EIRP
threshold depends on the audited quantity: thresholds derive from the
measured per-channel rms, not integration time, so every limit of
the released per-target file (Appendix~\ref{app:table}) is unaffected. Second, the audit is conservative
for every window of the two candidate hosts of
\S\ref{sec:technosearch} -- $\beta$~Pictoris's windows
(1824--1845\,s) sit inside an archive science exposure of
47\,444\,s, and AU~Mic's inside 109\,334\,s -- so no candidate
disposition changes. Third, the exposure-weighted coverage figure of
\S\ref{sec:technosearch} is at worst optimistic by the same two-block
margin: using the archive's values lowers the
$8.3\times10^{5}$\,s\,GHz total by less than 1\%. The residual
caveats motivate the direct per-EB export committed for the next
release. The defect class this audit caught was found by chance in
one target.
...
the released per-target file (Appendix~\ref{app:table}) allow rescaling to any other confidence level.
...
at these flux densities (\S\ref{sec:closurephase}). The
population-level anomaly screen (\S\ref{sec:popanomaly}) returns one
...
the released per-target file (Appendix~\ref{app:table}); filled circles are
...
per-target values of the released per-target file (Appendix~\ref{app:table}) are a snapshot that
...
the quoted EIRP$_{5\sigma}$ thresholds of the released per-target file (Appendix~\ref{app:table}) bound
...
searched grid and sacrificed by design (\S\ref{sec:extended} recovers
...
(\S\ref{sec:injectrecovery}), the probability of zero credible
...
integrates the injection-recovery curve of \S\ref{sec:injectrecovery}
...
the released per-target file (Appendix~\ref{app:table}); HN~Lib and LHS~1140, each host to a
...
continuum outlier (\S\ref{sec:popanomaly}) and no limit deriving from
...
by 7\%, \S\ref{sec:timeaudit}), the machine-readable value is the
...
trials of \S\ref{sec:injectrecovery}, including the 24-tone pilot);
...
\S\ref{sec:linecatalogue}; and (viii) the proper-motion-matched
...
marked $^{\ast}$ in the released per-target file (Appendix~\ref{app:table})); the median window is
...
The $\sigma_{\rm tot}$ column of the released per-target file (Appendix~\ref{app:table}) carries the map
...
not-applicable entry). Compiled from the released per-target file (Appendix~\ref{app:table}); see the reading rules
there.}
...
the corresponding entries in the released per-target file (Appendix~\ref{app:table}) is auditable.
