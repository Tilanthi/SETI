...
# make_fig_funnel.py must follow v381_calc.py: the decision-flow panel
# reads the outlier taxonomy macros that generator emits.
...
python3 audit_numbers_v384.py       > /dev/null   # forensic number audit; fails the build if any number is stale
