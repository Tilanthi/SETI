...
_POL = json.load(open('polstates_v351.json')) if os.path.exists(
    'polstates_v351.json') else None
if _POL:
    # Values are obscore pol_states strings such as '/XX/YY/'.
    _both = sum(1 for v in _POL.values()
                if 'XX' in str(v) and 'YY' in str(v))
...
_VIS = json.load(open('vistest_v384.json'))
# Take the keys from the JSON itself, ordered by the image-plane
# statistic, rather than hard-coding names that must match a suffix.
_ORDER = sorted((k for k, v in _VIS.items() if v.get('status') == 'ok'),
                key=lambda k: -_VIS[k]['event']['star_snr'])
with open('tab_visibility_v384.tex', 'w') as fh:
    fh.write('% generated by v381_calc.py -- do not edit\n')
    fh.write('\\begin{tabular}{@{}lrrrrrl@{}}\n\\toprule\n')
    fh.write('Star & $N_{\\rm vis}$ & Re/$\\sigma$ & Im/$\\sigma$ & '
             '$\\chi^2/\\nu$ & ctrl max & verdict \\\\\n\\midrule\n')
    _nv = 0
    for k in _ORDER:
        v = _VIS.get(k)
        ...
        if abs(st['snr_im']) > 3.0:
            verd = 'phase inconsistent with the star'
        elif st['snr_re'] <= v['ctrl_snr_re_max']:
            verd = 'not above its own controls'
        elif st['snr_re'] < 3.0:
            verd = 'no significant point source'
        else:
            verd = 'point source at the star'
            _nv += 1
        fh.write('%s & %s & $%+.2f$ & $%+.2f$ & %.2f & $%+.2f$ & %s \\\\\n'
                 % (tidy(k), '{:,}'.format(st['n_vis']).replace(',', '\\,'),
                    st['snr_re'], st['snr_im'], st['chi2_dof'],
                    v['ctrl_snr_re_max'], verd))
    fh.write('\\bottomrule\n\\end{tabular}\n')
assert len(_ORDER) == len(unattributed), (len(_ORDER), len(unattributed))
m('NVisTested', '%d' % len(_ORDER))
m('NVisPointSource', '%d' % _nv)
_hd = _VIS.get('HD14055', {}).get('star', {})
m('VisHdImSig', '%.1f' % abs(_hd.get('snr_im', 0)))
m('VisMaxReSig', '%.1f' % max(v['star']['snr_re'] for v in _VIS.values()
                              if v.get('status') == 'ok'))
...
_p90 = float(_texval('StratPNinety')) / 5.0
m('EirpNinetyMedA', _sci(_fine[len(_fine) // 2] * _p90))
m('EirpNinetyDeepA', _sci(_fine[0] * _p90))
...
_p90fac = float(_texval('StratPNinety')) / 5.0
...
if _hd:
    ...
    m('HdTsym', '%.2f' % F(_h['star_snr']))
    m('HdRingMax', '%.2f' % F(_h['ctrl_max_snr']))
    m('HdMargin', '%.2f' % (F(_h['star_snr']) - F(_h['ctrl_max_snr'])))
