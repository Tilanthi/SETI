...
CAT = 'per_target_results_v3.84.csv'
...
P90FAC = float(texval('StratPNinety')) / 5.0
...
ax = axes[0]
ax.step(p90, cum, where='post', color='#3a6ea5', lw=1.4)
ax.set_xscale('log')
ax.set_xlabel('$P_{90}$, 90 per cent recovery power (W)', fontsize=7.6)
ax.set_ylabel('independent systems with\nClass A coverage at or below', fontsize=7.6)
ax.tick_params(labelsize=6.8)
med = float(np.median(p90))
ax.axvline(med, color='#b03030', ls=':', lw=0.9)
ax.text(med * 1.15, cum[-1] * 0.10, 'median\n%s W' % ('%.1f' % (med/1e15) + r'$\times10^{15}$'), fontsize=6.2,
        color='#b03030')
ax.set_title('(a) Class A sensitivity, %d systems' % p90.size, fontsize=7.6, pad=3)
...
m('ClassAPNinetyMed', sci(med))
m('ClassAPNinetyBest', sci(float(p90.min())))
m('ClassAPNinetyWorst', sci(float(p90.max())))
m('EirpNinetyWorstA', sci(float(p90.max())))
