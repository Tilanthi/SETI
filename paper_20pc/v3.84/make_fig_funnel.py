...
PIPE = [
    ("%s windows" % n("NWindows"), "%s fine-channel, %s coarse-channel"
     % (n("NWinA"), n("NWinB"))),
    ("%s crossings" % n("NHits"), r"$T_\star \geq 5$ at the stellar position"),
    # Referee: the hold-out failure of exchangeability belongs in the
    # main candidate-flow figure, because it is what licenses calling this
    # step a prioritisation rather than a test.
    # One line: the box subtitle has no room for two, and a second line
    # overprinted the title.
    ("%s stage-1 flags" % n("NSpatial"),
     "star beats all %s controls; prioritisation only (rank median %s, not 0.5)"
     % (n("NCtrl"), n("HoRankMed"))),
    # This row was hand-typed as "3 identified astrophysical; 1
    # unexplained" and went stale the moment the catalogue grew. Read it
    # from the generated macros like every other number in the panel. The
    # panel is laid out for four boxes, so the split rides in the subtitle.
    ("%s persistent candidates" % n("NCandidates"),
     "%s CO-attributed, %s unexplained vs %s by chance"
     % (n("NStageOneLine"), n("NStageOneUnattrib"), n("ExpStageOneAll"))),
]
...
for i, (head, sub) in enumerate(PIPE):
    ...
    if i < 3:
        axR.add_patch(FancyArrowPatch((0.5, y1 - bh), (0.5, y1 - bh - bgap),
                                      ...
                                      color="0.35", lw=0.7))
GATES = ["spatial prioritisation", "line/instrumental vetting",
