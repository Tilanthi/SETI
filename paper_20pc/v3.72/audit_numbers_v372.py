...
R = list(csv.DictReader(open('per_target_results_v3.72.csv')))
...
ALL = {l.strip() for l in open('searched_ebs_v372.txt') if l.strip()}
...
try:
    ...
    figs = sorted(set(re.findall(r'\\includegraphics\[[^\]]*\]\{([^}]*)\}',
                                 open('technosignatures_40pc_v3.72.tex').read())))
    figreport = {}
    for g in figs:
