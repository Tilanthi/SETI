#!/usr/bin/env python3
"""Round-13 (v3.43) generated numbers.

Every quantity the three v3.43 referee reports showed to be hand-typed,
stale or contradicted by the shipped catalogue is recomputed here from
`per_target_results_v3.43.csv` (the released catalogue), the frozen export
...
CSV = HERE + '/per_target_results_v3.43.csv'
...
M('NSmearLo', '%d' % len(SM))
M('SmearPenMax', '%.2f' % (1.0 / f(SM[0], 'eta_smear')))
M('SmearEtaMin', '%.3f' % f(SM[0], 'eta_smear'))
WORST = [r for r in SM if f(r, 'eta_smear') < 0.7]
M('NSmearWorst', '%d' % len(WORST))
M('SmearWorstEtaLo', '%.3f' % min(f(r, 'eta_smear') for r in WORST))
M('SmearWorstEtaHi', '%.3f' % max(f(r, 'eta_smear') for r in WORST))
...
M('SmearEtaNextLo', '%.3f' % min(f(r, 'eta_smear') for r in SM
                                 if f(r, 'eta_smear') >= 0.7))
M('SmearEtaNextHi', '%.3f' % max(f(r, 'eta_smear') for r in SM
                                 if f(r, 'eta_smear') >= 0.7))
M('SmearPenNextMax', '%.2f' % (1.0 / min(f(r, 'eta_smear') for r in SM
                                         if f(r, 'eta_smear') >= 0.7)))
...
# G = exp(-4 ln2 (r/theta_PB)^2), theta_PB = 1.22 lambda/D.
def pbf(r):
...
M('PbMax', '%.2f' % pbf(TOP))
M('PbMaxPct', '%d' % round(100 * (pbf(TOP) - 1)))
...
M('MedWinFineLo', '%.0f' % min(r['_W'] for r in FINE))
M('MedWinFineHi', '%.0f' % max(r['_W'] for r in FINE))
M('MedWinCoarseLo', '%.0f' % min(r['_W'] for r in COARSE))
M('MedWinCoarseHi', '%.0f' % max(r['_W'] for r in COARSE))
M('NWinFullWidth', '%d' % sum(1 for r in ROWS if r['_pk']['nch'] > 2 * MEDWIN))
# the baseline ceiling W x channel width, per resolution class
ceilF = sorted(r['_W'] * f(r, 'chanw_Hz') for r in FINE)
ceilC = sorted(r['_W'] * f(r, 'chanw_Hz') for r in COARSE)
M('MedWinCeilFineMHz', '%.0f' % (st.median(ceilF) / 1e6))
M('MedWinCeilCoarseGHz', '%.1f' % (st.median(ceilC) / 1e9))
M('ChanFineKHz', '%.0f' % (st.median(f(r, 'chanw_Hz') for r in FINE) / 1e3))
M('ChanCoarseMHz', '%.2f' % (st.median(f(r, 'chanw_Hz') for r in COARSE) / 1e6))
...
M('CellsObsMedian', '%.1f' % st.median([r['_pk']['nhit'] for r in CRS]))
...
M('EdgeStarMin', tidy(EDGE[0][1]['star_name']))
...
NEFF7 = sorted(math.pi * (f(r, 'r_out_arcsec') ** 2 - f(r, 'r_in_arcsec') ** 2)
               ...
               if EBS.get(r['eb'], {}).get('s_resolution_arcsec'))
M('NeffMedSevenM', '%d' % round(st.median(NEFF7)))
